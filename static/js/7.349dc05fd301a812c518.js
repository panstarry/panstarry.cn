...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__staff_edit_vue__ = __webpack_require__(880);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__ = __webpack_require__(1122);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__pages_staff_staff_avatar_conf_vue__ = __webpack_require__(2303);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__ = __webpack_require__(1122);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__staff_salary_vue__ = __webpack_require__(1230);
...
/* harmony default export */ __webpack_exports__["default"] = ({
	...
	data() {
		return {
			...
			shopSptId: "",
			shopTarget: "",
			editable: $.Util.hasRoleFunc($.Dict.RoleFunc.PL_TARGET_CONF.value),
			messageConf: {
				isSmsNotice: null,
				isWxNotice: null,
				targetPercent: null,
	...
	created() {
		...
		years.map(yItem => {
			this.months.map(mitem => {
				this.dateList.push({ date: yItem + '年' + mitem + '月', to: yItem + '-' + mitem });
			});
		});
	...
	methods: {
		...
		savePlTarget(type) {
			...
			let args = {
				year: this.year,
				month: this.month,
				shopSptId: type == 0 ? this.shopSptId : "",
				shopTarget: type == 0 ? this.shopTarget : "",
				staffTargetList: type == 2 ? this.staffTargetList : [],
				pcGroupTargetList: type == 3 ? this.pcGroupTargetList : []
			};
			$.Req.service($.SvName.PL_TARGET_SAVE, args, res => {
				$.Msg.success("操作成功");
				this.loadTargetData();
			}, true);
		},
		...
		saveSettings() {
			...
			let args = {
				...
				sendDays: str,
				type: 2,
				moreStaffIds: conf.moreStaffIds.join(",")
			};
			$.Req.service($.SvName.TARGET_NOTICE_CONF_SAVE, args, res => {
				$.Msg.success("操作成功");
				this.loadSettingData();
			}, true);
		},
		...
		loadTargetData() {
			...
			$.Req.service($.SvName.PL_TARGET_QUERY, args, res => {
				this.shopName = res.shopName;
				this.shopSptId = res.shopSptId || "";
				this.shopTarget = res.shopTarget || "";
				this.staffTargetList = res.staffTargetList;
				this.pcGroupTargetList = res.pcGroupTargetList;
			}, true);
		},

		// 消息提醒查询
		loadSettingData() {
			$.Req.service($.SvName.TARGET_NOTICE_CONF_QUERY, { type: 2 }, res => {
				...
				if (res.moreStaffIds) {
					this.messageConf.moreStaffIds = res.moreStaffIds.split(",").map(m => Number(m));
				}
			}, true);
		},

		showCopy() {
			let d = { year: null, month: null };
			d.year = this.year;
			d.month = this.month < 9 ? '0' + this.month : this.month;
			this.srcYear = parseInt(d.year);
			this.srcMonth = parseInt(d.month);
			this.copyDlg = true;
		},
		...
		confirmCopy() {
			...
			let arg = {
				srcYear: this.srcYear,
				srcMonth: this.srcMonth,
				toDateList: []
			};
			this.copyList.map(item => {
				let date = item.split('-');
				arg.toDateList.push({ toYear: date[0], toMonth: date[1] });
			});
			$.Req.service($.SvName.PL_TARGET_COPY, arg, ret => {
				if (ret.msg) {
					$.Dlg.confirm(ret.msg, () => {
						arg.isCover = true;
						$.Req.service($.SvName.PL_TARGET_COPY, arg, ret => {
		...
		createGroup() {
			...
			this.group.remark = null;
			this.ownerStaffIdList = [];
			this.staffArr.map(sItem => {
				this.coachListFilt.map(c => {
					if (c.staffId == parseInt(sItem)) c.show = false;
				});
			});
			this.groupDlg = true;
		},
		...
		openEditDlg(idx) {
			...
			if (group.ownerStaffIds) {
				for (let val of group.ownerStaffIds.split(',')) {
					this.ownerStaffIdList.push(parseInt(val));
				}
				this.ownerStaffIdList.map(val => {
					this.coachListFilt.map(item => {
						if (val == item.staffId) item.show = true;
					});
				});
			}
			this.groupDlg = true;
		...
		submit() {
			...
			if (this.group.groupName.length > 50) {
				$.Dlg.error('分组名称不能超过50字');
				return;
			}
			this.group.ownerStaffIds = $.Util.isEmptyArray(this.ownerStaffIdList) ? null : this.ownerStaffIdList.join(',');

			let args = {
				actionType: this.group.groupId ? 'U' : 'I',
				group: this.group
			};
			$.Req.service($.SvName.PC_GROUP_SAVE, args, ret => {
				$.Msg.success($.Lang.SAVE_SUCCESS);
				this.initData();
				this.groupDlg = false;
			});
		...
		queryShopGroupList() {
			$.Req.service($.SvName.PC_GROUP_QUERY, {}, ret => {
				this.groupList = ret.groupList;
				if (this.groupList && this.groupList.length) {
					let staffIds = '';
					this.groupList.map((item, idx) => {
						staffIds += item.ownerStaffIds + ',';
					});
					let staffArr = staffIds.split(',');
					let uniqueArray = arr => {
						return arr.filter((item, index) => {
							return arr.indexOf(item) === index;
						});
					};
					staffArr = uniqueArray(staffArr);
					staffArr.pop();
					staffArr.map(sItem => {
						this.coachListFilt.map(c => {
							if (c.staffId == parseInt(sItem)) c.show = false;
		...
		delGroup(idx) {
			let args = {
				actionType: 'D',
				deleteId: this.groupList[idx].groupId
			};
			$.Req.service($.SvName.PC_GROUP_SAVE, args, ret => {
				if (ret.msg) {
					$.Dlg.confirm(ret.msg, yes => {
						args.isDelete = true;
						$.Req.service($.SvName.PC_GROUP_SAVE, args, ret => {
							$.Msg.success('删除成功');
							this.initData();
		...
		queryStaffList(callback = null) {
			let args = { isPrivateCoach: true };
			$.Req.service($.SvName.COACH_BASE_QUERY, args, ret => {
				this.coachListFilt = [];
				for (let s of ret.coachList) {
					this.staffNameMap[s.staffId] = s.name;
					s.show = true;
					this.coachListFilt.push(s);
				}
				if (callback) callback();
			});``;
		},
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__staff_view_vue__ = __webpack_require__(2304);
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    // 消息提醒查询
    loadSettingData() {
      $.Req.service($.SvName.TARGET_NOTICE_CONF_QUERY, { type: 1 }, res => {
        this.messageConf = res;
        if (res.targetRate && res.targetRate > 0) {
          this.messageConf.targetPercent = res.targetRate * 100;
        }
    ...
    confirmCopy() {
      let srcYear, srcMonth;
      if (!this.copyDate) {
        $.Msg.warning('请选择年月');
        return;
      } else {
        srcYear = this.copyDate.getFullYear();
        srcMonth = this.copyDate.getMonth() + 1;
      }
      let arg = {
        srcYear: srcYear,
        srcMonth: srcMonth,
      ...
      $.Req.service($.SvName.SALES_TARGET_COPY, arg, ret => {
        if (ret.msg) {
          $.Dlg.confirm(ret.msg, () => {
            arg.isCover = true;
            $.Req.service($.SvName.PL_TARGET_COPY, arg, ret => {
...
// module
exports.push([module.i, ".sec-box[data-v-2152f950]{margin-top:-12px;max-width:1100px;display:flex;flex-wrap:wrap}.sec-box .sec-item[data-v-2152f950]{width:160px;margin-top:20px}.sec-box .sec-item input[data-v-2152f950]{width:120px}.shop-setting .sec-item[data-v-2152f950]{width:100%;width:auto;flex-direction:row;align-items:center}.shop-setting .sec-item span[data-v-2152f950]{width:auto;margin-bottom:0}.sec-item[data-v-2152f950]{display:flex;flex-direction:column;align-items:flex-start;overflow:hidden}.sec-item span[data-v-2152f950]{display:inline-block;width:100%;padding-right:20px;margin-bottom:10px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}.sec-item button[data-v-2152f950]{margin-left:20px}.sec-item[data-v-2152f950] .ivu-input-wrapper{width:120px}.sec-item[data-v-2152f950] .ivu-date-picker-editor{width:200px}.settings input[type=number][data-v-2152f950]{border:0;border-bottom:1px solid #ddd;width:80px;text-align:center;height:24px;border-radius:0;display:inline-block;margin:0 5px;font-size:15px;font-weight:700;outline:none}.settings section>ol[data-v-2152f950],.settings section>ul[data-v-2152f950]{padding-left:5px}.settings section>ol>li[data-v-2152f950],.settings section>ul>li[data-v-2152f950]{padding:10px 5px 10px 12px;border-bottom:1px dashed #eee;width:900px}.settings section>ol>li .tips[data-v-2152f950],.settings section>ul>li .tips[data-v-2152f950]{padding-left:15px;color:#999;font-size:12px}.settings section>ol>li:hover input[data-v-2152f950],.settings section>ol>li[data-v-2152f950]:hover,.settings section>ul>li:hover input[data-v-2152f950],.settings section>ul>li[data-v-2152f950]:hover{background:#f9f9f9}.settings section>ol>li:hover .tips[data-v-2152f950],.settings section>ul>li:hover .tips[data-v-2152f950]{color:#ff4500}.settings section>ol>li .tit[data-v-2152f950],.settings section>ul>li .tit[data-v-2152f950]{display:inline-block;width:120px}.settings section>ol>li label[data-v-2152f950],.settings section>ul>li label[data-v-2152f950]{margin-right:15px;font-size:13px}.remark[data-v-2152f950]{margin-top:15px;font-size:12px;color:#ff4500}.query-btn[data-v-2152f950]{width:75px;height:24px;padding:0}.staff-names li[data-v-2152f950]{display:inline;border-right:1px solid #ddd;padding:0 10px 0 12px}.staff-names li[data-v-2152f950]:first-child{padding-left:0}.staff-names li[data-v-2152f950]:last-child{border-right:none}.ivu-tabs[data-v-2152f950]{overflow:initial}.msg-more-staff[data-v-2152f950]{width:560px;display:flex;align-items:center}.msg-more-staff .ivu-select[data-v-2152f950]{flex:1}.msg-more-staff .mytips[data-v-2152f950]{width:80px;display:flex;align-items:center;justify-content:flex-end;margin-bottom:3px}", ""]);
...
// module
exports.push([module.i, ".date-conf[data-v-7aad01e8]{margin:15px 0}.date-conf .tit[data-v-7aad01e8]{color:#999;padding-left:5px}.sec-box[data-v-7aad01e8]{margin-top:-12px;max-width:1100px;display:flex;flex-wrap:wrap}.sec-box .sec-item[data-v-7aad01e8]{width:160px;margin-top:20px}.sec-box .sec-item input[data-v-7aad01e8]{width:120px}.shop-setting .sec-item[data-v-7aad01e8]{width:100%;width:auto;flex-direction:row;align-items:center}.shop-setting .sec-item span[data-v-7aad01e8]{width:auto;margin-bottom:0}.sec-item[data-v-7aad01e8]{display:flex;flex-direction:column;align-items:flex-start;overflow:hidden}.sec-item span[data-v-7aad01e8]{display:inline-block;width:100%;padding-right:20px;margin-bottom:10px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}.sec-item button[data-v-7aad01e8]{margin-left:20px}.sec-item[data-v-7aad01e8] .ivu-input-wrapper{width:120px}.sec-item[data-v-7aad01e8] .ivu-date-picker-editor{width:200px}.settings section>ol[data-v-7aad01e8],.settings section>ul[data-v-7aad01e8]{padding-left:5px}.settings section>ol>li[data-v-7aad01e8],.settings section>ul>li[data-v-7aad01e8]{padding:10px 5px 10px 12px;border-bottom:1px dashed #eee;width:900px}.settings section>ol>li .tips[data-v-7aad01e8],.settings section>ul>li .tips[data-v-7aad01e8]{padding-left:15px;color:#999;font-size:12px}.settings section>ol>li:hover input[data-v-7aad01e8],.settings section>ol>li[data-v-7aad01e8]:hover,.settings section>ul>li:hover input[data-v-7aad01e8],.settings section>ul>li[data-v-7aad01e8]:hover{background:#f9f9f9}.settings section>ol>li:hover .tips[data-v-7aad01e8],.settings section>ul>li:hover .tips[data-v-7aad01e8]{color:#ff4500}.settings section>ol>li .tit[data-v-7aad01e8],.settings section>ul>li .tit[data-v-7aad01e8]{display:inline-block;width:120px}.settings section>ol>li label[data-v-7aad01e8],.settings section>ul>li label[data-v-7aad01e8]{margin-right:15px;font-size:13px}.settings input[type=number][data-v-7aad01e8]{border:0;border-bottom:1px solid #ddd;width:80px;text-align:center;height:24px;border-radius:0;display:inline-block;margin:0 5px;font-size:15px;font-weight:700;outline:none}.remark[data-v-7aad01e8]{margin-top:15px;font-size:12px;color:#ff4500}.query-btn[data-v-7aad01e8]{width:120px}.ivu-tabs[data-v-7aad01e8]{overflow:initial}.msg-more-staff[data-v-7aad01e8]{width:560px;display:flex;align-items:center}.msg-more-staff .ivu-select[data-v-7aad01e8]{flex:1}.msg-more-staff .mytips[data-v-7aad01e8]{width:80px;display:flex;align-items:center;justify-content:flex-end;margin-bottom:3px}", ""]);
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("0a49ded0", content, true, {});
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('shop-nav', {
  ...
  }, [_vm._v("复制")]) : _vm._e()], 1)]), _vm._v(" "), _c('div', [_c('Date-picker', {
    staticStyle: {
      "width": "120px"
    },
    attrs: {
  ...
  })], 1)])]), _vm._v(" "), (_vm.pcGroupTargetList.length) ? _c('section', {
    staticClass: "conf"
  }, [_c('div', {
    staticClass: "hd"
  }, [_c('div', {
    staticClass: "tit"
  }, [_vm._v("分组目标设置")]), _vm._v(" "), _c('div', {
    staticClass: "r"
  }, [(_vm.editable) ? _c('i-button', {
    staticClass: "query-btn",
    attrs: {
    ...
    on: {
      "click": function($event) {
        return _vm.savePlTarget(3)
      }
    }
  }, [_vm._v("保存")]) : _vm._e()], 1)]), _vm._v(" "), _c('div', {
    staticClass: "sec-box"
  }, _vm._l((_vm.pcGroupTargetList), function(item, idx) {
    return _c('div', {
      key: idx,
      staticClass: "sec-item"
    }, [_c('span', {
    ...
    }, [_vm._v(_vm._s(item.groupName))]), _vm._v(" "), _c('input-num', {
      attrs: {
        "disabled": !_vm.editable,
        "placeholder": "请输入目标"
      },
  ...
  }) : _c('b', [_vm._v(_vm._s(_vm.messageConf.sendDaysArr[2]))])], 1), _vm._v(" "), _c('li', [_c('div', {
    staticClass: "msg-more-staff"
  }, [_c('span', {
    staticStyle: {
      "width": "120px"
    }
  }, [_vm._v("额外通知人员")]), _vm._v(" "), _c('i-select', {
    attrs: {
      "disabled": !_vm.editable,
      "placeholder": "请选择额外通知人员（可多选）",
      "multiple": "",
      "filterable": ""
    },
    model: {
      value: (_vm.messageConf.moreStaffIds),
      callback: function($$v) {
        _vm.$set(_vm.messageConf, "moreStaffIds", $$v)
      },
      expression: "messageConf.moreStaffIds"
    }
  ...
  }, [_vm._v("通知除了发送给低于目标的员工，还可设置额外发送给管理人员（例如，超管，店长角色的员工）")])], 1)])]), _vm._v(" "), _c('div', {
    staticClass: "remark"
  }, [_c('b', [_vm._v("备注：")]), _vm._v("在上述日期实时检查，对低于耗课目标的耗课人员发送通知提醒\n              ")]), _vm._v(" "), (_vm.editable) ? _c('i-button', {
    staticStyle: {
      "margin": "20px 0 5px 0",
      "width": "120px"
    },
  ...
  }, [_vm._v("保存")]) : _vm._e()], 1)])]), _vm._v(" "), _c('TabPane', {
    attrs: {
      "label": "私教分组"
    }
  }, [_c('section', [_c('div', {
  ...
  }, [_vm._v("添加分组")]) : _vm._e()], 1)]), _vm._v(" "), _c('div', {
    staticClass: "mytbl"
  }, [_c('table', [_c('tr', [_c('th', {
    attrs: {
      "width": "45"
    }
  }, [_vm._v("#")]), _vm._v(" "), _c('th', [_vm._v("分组名称")]), _vm._v(" "), _c('th', [_vm._v("分组员工")]), _vm._v(" "), _c('th', [_vm._v("备注")]), _vm._v(" "), _c('th', [_vm._v("更新人")]), _vm._v(" "), _c('th', [_vm._v("更新时间")]), _vm._v(" "), _c('th', {
    attrs: {
      "width": "100"
    }
  }, [_vm._v("操作")])]), _vm._v(" "), _vm._l((_vm.groupList), function(d, idx) {
    return _c('tr', [_c('td', {
      staticClass: "gray"
    }, [_vm._v(_vm._s(idx + 1))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(d.groupName))]), _vm._v(" "), _c('td', [(d.ownerStaffIds) ? _c('ul', {
      ...
      on: {
        "click": function($event) {
          return _vm.delGroup(idx)
        }
      }
    }, [_c('i', {
      staticClass: "iconfont icon-delete normal"
    })])])]) : _vm._e()])])
  })], 2), _vm._v(" "), (_vm.groupList && !_vm.groupList.length) ? _c('div', {
    staticClass: "no-data"
  ...
  }, [_c('div', [_c('p', [_vm._v("将当前选择月份 "), _c('b', {}, [_vm._v(_vm._s(_vm.srcYear) + "年" + _vm._s(_vm.srcMonth) + "月")]), _vm._v(" 的目标数据复制到以下月份：")]), _vm._v(" "), _c('i-select', {
    staticStyle: {
      "width": "600px"
    },
    attrs: {
  ...
  }, _vm._l((_vm.dateList), function(d, idx) {
    return _c('i-option', {
      key: idx,
      staticStyle: {
        "width": "500px"
    ...
    model: {
      ...
      callback: function($$v) {
        _vm.$set(_vm.group, "groupName", (typeof $$v === 'string' ? $$v.trim() : $$v))
      },
      expression: "group.groupName"
    }
  })], 1)]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("分组员工")]), _vm._v(" "), _c('td', [_c('i-select', {
    attrs: {
      "placeholder": "请选择分组员工（可多选）",
      "multiple": "",
      "filterable": ""
    },
    model: {
  ...
  }, _vm._l((_vm.coachListFilt), function(s) {
    return (s.show) ? _c('i-option', {
      key: s.staffId,
      attrs: {
        "value": s.staffId
      }
    }, [_vm._v(_vm._s(s.name))]) : _vm._e()
  }), 1)], 1)]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("备注")]), _vm._v(" "), _c('td', [_c('i-input', {
    attrs: {
      "type": "textarea",
      "rows": 5,
      "maxlength": 200,
    ...
    model: {
      value: (_vm.group.remark),
      callback: function($$v) {
        _vm.$set(_vm.group, "remark", (typeof $$v === 'string' ? $$v.trim() : $$v))
      },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  ...
  }, [_vm._v("复制销售目标数据")])], 1)]), _vm._v(" "), _c('div', [_c('Date-picker', {
    staticStyle: {
      "width": "120px"
    },
    attrs: {
  ...
  }, [_vm._v("保存")]) : _vm._e()], 1)])])], 1)], 1), _vm._v(" "), _c('Modal', {
    attrs: {
      "title": "复制销售目标数据",
      "width": "400",
      "cancelText": "",
  ...
  }), _vm._v("\n\t\t\t  复制目标数据到 " + _vm._s(Date.new(_vm.queryDate).format('yyyy-M')) + "月\n\t\t  ")], 1) : _vm._e(), _vm._v(" "), _c('div', {
    staticClass: "center",
    staticStyle: {
      "padding-bottom": "10px"
    },
