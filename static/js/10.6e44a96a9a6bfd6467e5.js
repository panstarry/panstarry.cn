...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  data() {
    return {
      g: window.$,
      mallOrderRefundRole: $.Util.hasRoleFunc($.Dict.RoleFunc.MALL_ORDER_REFUND.value),
      refundList: [],
      orderCount: null,
      refund: null,
      order: null,
  ...
  watch: {},
  created() {
    if (!this.mallOrderRefundRole) return;
    this.orderRefundQuery();
  },
  mounted() {},
  methods: {
    viewMember(memberId) {
    ...
    orderRefundQuery(page = null) {
      const args = {
        page: page
      };
      $.Req.service($.SvName.MALL_ORDER_PRODUCT_REFUND_QUERY, args, ret => {
        this.refundList = ret.refundList;
        this.orderCount = ret.count;
      });
    },

    orderRefundQueryItem(mopId) {
      const args = {
        mopId: mopId
      };
      $.Req.service($.SvName.MALL_ORDER_PRODUCT_REFUND_QUERY, args, ret => {
        this.refund = ret.refund;
        this.selectOrder = ret.order;
        this.fill.status = ret.refund.status;
      });
    },

    colorChangeClassFun(type) {
      if (type === 0) {
        return "orange";
      } else if (type === 1) {
        return "red";
    ...
    refundOrderFun(order) {
      this.orderRefundQueryItem(order.mopId);
      this.selectOrderDlg = true;
    },

    updateOrderRefundFun() {
      if (!this.fill.status) {
        return alert("请选择是否同意");
      }
      const args = {
        mopId: this.refund.mopId,
        status: this.fill.status,
        refuseRemark: this.fill.refuseRemark
      };
      $.Req.service($.SvName.MALL_ORDER_PRODUCT_REFUND_UPDATE, args, ret => {
        $.Msg.success($.Lang.OPT_SUCCESS);
        this.orderRefundQuery();
        this.selectOrderDlg = false;
      });
    },

    refuseRemarkFun() {
      this.refuseShow = true;
      if (this.refund && this.refund.refuseRemark) {
        this.fill.refuseRemark = this.refund.refuseRemark;
      }
    },
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__order_refund_vue__ = __webpack_require__(2256);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__order_refund_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2__order_refund_vue__);
//
//
//
//
...
/* harmony default export */ __webpack_exports__["default"] = ({
	components: { MemberBaseInfo: __WEBPACK_IMPORTED_MODULE_0__pages_member_member_base_info_vue___default.a, MemberSelect: __WEBPACK_IMPORTED_MODULE_1__pages_member_member_select_vue___default.a, OrderRefund: __WEBPACK_IMPORTED_MODULE_2__order_refund_vue___default.a },
	data() {
		return {
			g: $,
			orderCancelPermit: $.Util.hasRoleFunc($.Dict.RoleFunc.MALL_ORDER_REFUND.value),
			orderList: null,
			orderCount: null,
			viewMemberId: null,
			viewMemberDlg: false,
	...
	methods: {
		...
		// 优惠券
		quyeryCouponLogList(p) {
			const args = { memberId: p.memberId, useRelType: 2, useRelId: p.mopId };
			$.Req.service($.SvName.USE_COUPON_LOG_QUERY, args, ret => {
				this.couponLogList = ret.logList;
				this.useCouponDlg = true;
		...
		queryShopCoupon() {
			$.Req.service($.SvName.ACTIVITY_COUPON_QUERY, {}, ret => {
				if (ret.couponList && ret.couponList.length) {
					this.isOpenShopCoupon = true;
				}
			});
		}
	}
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__ = __webpack_require__(1122);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__mall_product_log_vue__ = __webpack_require__(2255);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_qrcode_vue__ = __webpack_require__(1133);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_html2canvas__ = __webpack_require__(1151);
...
/* harmony default export */ __webpack_exports__["default"] = ({
	components: { TenantMpAuth: __WEBPACK_IMPORTED_MODULE_3__tenant_tenant_mp_auth___default.a, TimeSelect: __WEBPACK_IMPORTED_MODULE_2__components_sys_time_select___default.a, PageHeader: __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue___default.a, MallProductLog: __WEBPACK_IMPORTED_MODULE_1__mall_product_log_vue___default.a, QrcodeVue: __WEBPACK_IMPORTED_MODULE_4_qrcode_vue__["a" /* default */] },
	data() {
		return {
			g: window.$,
			editable: $.Util.hasRoleFunc($.Dict.RoleFunc.MALL_PRODUCT_EDIT.value),
			...
			productLinkAndQRCodeDlg: false,
			selectProduct: {},
			selectProductUrl: null

		};
	},

	watch: {
		productLinkAndQRCodeDlg(newVal) {
			if (!newVal) this.selectProductUrl = null;
		}
	},
	...
	methods: {
		...
		queryProduct(page = null) {
			...
			let args = {
				...
				productName: this.query.productName === '' ? null : this.query.productName
			};
			$.Req.service($.SvName.MALL_PRODUCT_QUERY, args, ret => {
				if (!page) {
		...
		checkProduct(prdId) {
			if (this.selectProdIdList.contains(prdId)) {
				this.selectProdIdList.removeValue(prdId);
			} else {
				this.selectProdIdList.push(prdId);
			}
		},
		...
		selectAll() {
			...
			if (this.isSelectAll) {
				for (let p of this.productList) {
					this.selectProdIdList.push(p.productId);
				}
			} else {
				this.selectProdIdList = [];
			}
		},

		openProductCopyDlg() {
			if (this.selectProdIdList.length === 0) {
				$.Msg.error('请选择需要复制到商品');
				return;
			}

			this.prodCopyDlg = true;
		},
		...
		copyProdctToShops() {
			if (this.selectEncSidList.length === 0) {
				$.Msg.error('请选择复制到门店');
				return;
			}

			let args = {
				encSidList: this.selectEncSidList,
				productIdList: this.selectProdIdList
			};
			$.Req.service($.SvName.MALL_PRODUCT_COPY, args, ret => {
				$.Msg.success($.Lang.OPT_SUCCESS);
				this.prodCopyDlg = false;
			}, true);
		},

		openProductLinkDlg(p) {
			this.selectProduct = Object.assign({
				shareLinkUrl: `${$.Conf.MOBILE_CLIENT_URL}#/mall/product/${p.productId}?encSid=${encodeURIComponent($.data.shop.encSid)}`
			}, p);
			this.productLinkAndQRCodeDlg = true;
			this.$nextTick(() => {
		...
		downloadAsImage() {
			__WEBPACK_IMPORTED_MODULE_5_html2canvas___default()(this.$refs.myQrCode, {
				scale: 2, // 设置缩放比例，保证清晰度
				logging: false // 关闭日志输出
			}).then(canvas => {
				var imgData = canvas.toDataURL('image/png');
				this.selectProductUrl = imgData;
			});
		}
	}
});
...
// module
exports.push([module.i, "[data-v-07c36a07].ivu-modal-footer{padding-top:0}.unpay[data-v-07c36a07]{color:red}.orders .prds li[data-v-07c36a07]{display:inline-block;padding-right:5px}.orders .prds li[data-v-07c36a07]:after{content:\"\\3001\"}.orders .prds li[data-v-07c36a07]:last-child{padding:0}.orders .prds li[data-v-07c36a07]:last-child:after{content:\"\"}.order-detail .prds li[data-v-07c36a07]{display:block;list-style:circle;padding-bottom:8px;position:relative}.order-detail .prds li[data-v-07c36a07]:last-child{padding-bottom:0}.order-detail .prds li .right[data-v-07c36a07]{width:200px;position:absolute;right:0}.footer-btns[data-v-07c36a07]{margin-top:10px}", ""]);
...
// module
exports.push([module.i, ".warning[data-v-1955357e]{margin:15px 0;text-align:center;line-height:55px;height:55px;padding:0;border:1px solid #ddd;background:#fafafa}.note li[data-v-1955357e]{font-size:12px}.shop-list[data-v-1955357e]{margin-top:10px}.shop-list label[data-v-1955357e]{display:block;margin:5px 0;padding:5px 0}.card[data-v-1955357e]{background:#eaeaea;color:#555;padding:1px 6px;border-radius:2px;margin-left:8px}.qrcode img[data-v-1955357e]{width:150px}.prdlink td[data-v-1955357e]{padding:8px 10px;min-width:110px}.prdlink h3[data-v-1955357e]{margin:0;font-size:14px}.prdlink .qrcode[data-v-1955357e]{width:200px;border:1px solid #ddd}.prdlink .link-value[data-v-1955357e]{background:#eee;width:500px}.link-value[data-v-1955357e]{display:flex;padding:3px 10px}.link-value .link[data-v-1955357e]{flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.link-value .copy-tip[data-v-1955357e]{width:40px;text-align:right;display:none}.link-value[data-v-1955357e]:hover{background:#eee;cursor:pointer;position:relative}.link-value:hover .copy-tip[data-v-1955357e]{display:inline-block}.link-value:active .copy-tip[data-v-1955357e]{color:red}.row-sb[data-v-1955357e]{display:flex;justify-content:space-between}.qr-box[data-v-1955357e]{border:1px solid #ddd;display:inline-block}.qr-box .qr-title[data-v-1955357e]{font-size:15px;text-align:center;margin-bottom:15px;font-weight:700}.qr-box .qr-code-img[data-v-1955357e]{width:200px;padding:20px 20px 15px;padding-bottom:17px;vertical-align:middle}.qr-box .qr-code-img canvas[data-v-1955357e]{vertical-align:middle}.qr-box .qr-code-img .qr-staff-name[data-v-1955357e]{text-align:center;font-size:13px;margin-top:10px;font-weight:700}.qr-box .qr-code-img-url[data-v-1955357e]{width:200px}", ""]);
...
// module
exports.push([module.i, ".unpay[data-v-6db0f104]{color:red}.orders .prds li[data-v-6db0f104]{display:inline-block;padding-right:5px}.orders .prds li[data-v-6db0f104]:after{content:\"\\3001\"}.orders .prds li[data-v-6db0f104]:last-child{padding:0}.orders .prds li[data-v-6db0f104]:last-child:after{content:\"\"}.order-detail .prds li[data-v-6db0f104]{display:block;list-style:circle;padding-bottom:8px}.order-detail .prds li[data-v-6db0f104]:last-child{padding-bottom:0}.use-coupon-dlg table[data-v-6db0f104]{margin-bottom:20px}.use-coupon-dlg table[data-v-6db0f104]:last-of-type{margin-bottom:0}", ""]);
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("282879f2", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("60f71356", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("c774a67c", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("7200b8d8", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("0da9b7ef", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("60916193", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("054b11a4", content, true, {});
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: "-order-refund"
  }, [(!_vm.mallOrderRefundRole) ? _c('section', {
    staticClass: "no-permit"
  }, [_c('i', {
    staticClass: "iconfont icon-alert2 big"
  }), _vm._v(" 您尚无【" + _vm._s(_vm.g.Dict.RoleFunc.MALL_ORDER_REFUND.name) + "】权限，无权查看\n  ")]) : [_c('section', [_c('div', {
    staticClass: "mytbl orders"
  }, [_c('table', [_vm._m(0), _vm._v(" "), _vm._l((_vm.refundList), function(o, idx) {
    return _c('tr', {
      key: idx
    }, [_c('td', [_vm._v("\n              " + _vm._s(o.refundNo) + "\n            ")]), _vm._v(" "), _c('td', [_c('span', {
      class: _vm.colorChangeClassFun(o.status)
    }, [_vm._v("\n                " + _vm._s(_vm.g.Dict.refundStatus[o.status]) + "\n              ")])]), _vm._v(" "), _c('td', [_vm._v("\n              " + _vm._s(o.productName) + "\n            ")]), _vm._v(" "), _c('td', [_c('a', {
      staticClass: "block",
      on: {
        "click": function($event) {
    ...
    }, [_vm._v(_vm._s(o.memberName))])]), _vm._v(" "), _c('td', [(o.paymentPoints) ? [_vm._v(_vm._s(o.paymentPoints))] : _c('i', {
      staticClass: "gray"
    }, [_vm._v("无")])], 2), _vm._v(" "), _c('td', [(o.paymentAmount) ? [_vm._v("￥" + _vm._s(o.paymentAmount))] : _c('i', {
      staticClass: "gray"
    }, [_vm._v("无")])], 2), _vm._v(" "), _c('td', [_vm._v("\n              " + _vm._s(o.createTime) + "\n            ")]), _vm._v(" "), _c('td', [_vm._v("\n              " + _vm._s(o.applyRemark) + "\n            ")]), _vm._v(" "), _c('td', [_vm._v("\n              " + _vm._s(o.updateUname) + "\n            ")]), _vm._v(" "), _c('td', [_vm._v("\n              " + _vm._s(o.updateTime) + "\n            ")]), _vm._v(" "), _c('td', [_vm._v("\n              " + _vm._s(o.unitNum) + "\n            ")]), _vm._v(" "), _c('td', [_c('a', {
      on: {
        "click": function($event) {
          return _vm.refundOrderFun(o)
        }
      }
    }, [_vm._v("详情")])])])
    ...
    on: {
      "on-change": _vm.orderRefundQuery
    }
  }) : _c('div', {
    staticClass: "center gray"
  }, [_vm._v("共 " + _vm._s(_vm.orderCount) + " 条记录")])], 1) : _vm._e(), _vm._v(" "), (_vm.refundList && !_vm.refundList.length) ? _c('div', {
    staticClass: "no-data"
  ...
  }, [_c('table', [_c('tr', [_c('th', [_vm._v("退款编号")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.refund.refundNo))])]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("申请时间")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.refund.createTime))])]), _vm._v(" "), _c('tr', [_c('th', {
    attrs: {
      "width": "100"
    }
  }, [_vm._v("买家信息")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.refund.memberName))])]), _vm._v(" "), (_vm.refund.paymentAmount >= 0) ? _c('tr', [_c('th', {
    attrs: {
      "width": "100"
    }
  }, [_vm._v("退款金额")]), _vm._v(" "), _c('td', [_vm._v("￥" + _vm._s(_vm.refund.paymentAmount))])]) : _vm._e(), _vm._v(" "), (_vm.refund.paymentPoints) ? _c('tr', [_c('th', {
    attrs: {
      "width": "100"
    }
  }, [_vm._v("退款积分")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.refund.paymentPoints) + " 分")])]) : _vm._e(), _vm._v(" "), _c('tr', [_c('th', [_vm._v("退款原因")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.refund.applyRemark))])]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("退款商品")]), _vm._v(" "), _c('td', [_c('ul', {
    staticClass: "prds"
  }, [_c('li', [_vm._v("\n                ※ " + _vm._s(_vm.refund.productName) + " × " + _vm._s(_vm.refund.unitNum) + "\n                "), _c('span', {
    staticClass: "l10"
  }, [(_vm.refund.status === 9 && (_vm.refund.refundCardAmount || _vm.refund.refundCashAmount)) ? [_vm._v("\n                    （"), (_vm.refund.refundCardAmount) ? [_vm._v("储值卡退："), _c('span', {
    staticClass: "bold"
  }, [_vm._v(_vm._s(_vm.refund.refundCardAmount) + " ")]), _vm._v("元")] : _vm._e(), _vm._v(" "), (_vm.refund.refundCardAmount && _vm.refund.refundCashAmount) ? [_vm._v("; ")] : _vm._e(), _vm._v(" "), (_vm.refund.refundCashAmount) ? [_vm._v("微信退："), _c('span', {
    staticClass: "bold"
  }, [_vm._v(_vm._s(_vm.refund.refundCashAmount) + " ")]), _vm._v("元")] : _vm._e(), _vm._v("）\n                  ")] : _vm._e()], 2), _vm._v(" "), (_vm.refund.status === 0) ? [_c('span', {
    staticClass: "right"
  }, [_c('RadioGroup', {
    model: {
      value: (_vm.fill.status),
      callback: function($$v) {
        _vm.$set(_vm.fill, "status", $$v)
      },
      expression: "fill.status"
    }
  }, [_c('Radio', {
    attrs: {
      "label": 1
    }
  }, [_vm._v("拒绝")]), _vm._v(" "), _c('Radio', {
    attrs: {
      "label": 9
    }
  }, [_vm._v("同意")])], 1)], 1), _vm._v(" "), (_vm.fill.status === 1) ? _c('a', {
    on: {
      "click": _vm.refuseRemarkFun
    }
  }, [_vm._v("原因")]) : _vm._e()] : _vm._e()], 2)])])]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("购买商品")]), _vm._v(" "), _c('td', [_c('ul', {
    staticClass: "prds"
    ...
    }, [_vm._v(" ＋ ")]) : _vm._e(), _vm._v(" "), (p.priceType === 1 || p.priceType === 2) ? [_vm._v("￥" + _vm._s(p.priceAmount))] : _vm._e()], 2), _vm._v("\n                  ）\n                ")]), _vm._v(" "), (p.refundStatus === 9) ? _c('span', {
      staticClass: "gray"
    }, [_vm._v("（已退）")]) : _vm._e()])
  }), 0)])]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("退款状态")]), _vm._v(" "), _c('td', [_c('span', {
    class: _vm.colorChangeClassFun(_vm.refund.status)
  }, [_vm._v("\n              " + _vm._s(_vm.g.Dict.refundStatus[_vm.refund.status]) + "\n            ")])])]), _vm._v(" "), (_vm.refund.refuseRemark) ? _c('tr', [_c('th', [_vm._v("退款拒绝原因")]), _vm._v(" "), _c('td', [_vm._v("\n            " + _vm._s(_vm.refund.refuseRemark) + "\n          ")])]) : _vm._e()]), _vm._v(" "), _c('div', {
    staticClass: "center footer-btns"
  }, [(_vm.refund.status === 0) ? _c('i-button', {
    staticStyle: {
      "width": "100px",
      "margin-right": "10px"
    },
    ...
    on: {
      "click": _vm.updateOrderRefundFun
    }
  }, [_vm._v("确定")]) : _vm._e(), _vm._v(" "), _c('i-button', {
    on: {
      "click": function($event) {
        _vm.selectOrderDlg = false
      }
    }
  }, [_vm._v("关闭")])], 1)]) : _vm._e()]), _vm._v(" "), _c('modal', {
    attrs: {
      "reset-drag-position": "",
      "title": "拒绝退款原因",
      "width": "350",
    ...
    model: {
      value: (_vm.refuseShow),
      callback: function($$v) {
        _vm.refuseShow = $$v
      },
      expression: "refuseShow"
    }
  }, [_c('i-input', {
    attrs: {
      "show-word-limit": "",
    ...
    model: {
      value: (_vm.fill.refuseRemark),
      callback: function($$v) {
        _vm.$set(_vm.fill, "refuseRemark", $$v)
      },
      expression: "fill.refuseRemark"
    }
  })], 1), _vm._v(" "), _c('modal', {
    attrs: {
      "title": "会员基本信息",
  ...
  }, [_vm._v("退款积分")]), _vm._v(" "), _c('th', {
    attrs: {
      "width": "80"
    }
  }, [_vm._v("退款金额")]), _vm._v(" "), _c('th', {
    attrs: {
      "width": "150"
    }
  }, [_vm._v("申请时间")]), _vm._v(" "), _c('th', [_vm._v("退款原因")]), _vm._v(" "), _c('th', {
    attrs: {
      "width": "110"
    }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('shop-nav', {
  ...
  }, _vm._l((_vm.g.Dict.MallProductStatuses), function(s, idx) {
    return _c('i-option', {
      key: idx,
      attrs: {
        "value": idx
  ...
  }, _vm._l((_vm.g.Dict.MallProductTypes), function(v, idx) {
    return _c('i-option', {
      key: idx,
      attrs: {
        "value": idx
  ...
  }, _vm._l((_vm.g.Dict.MallProductPriceTypes), function(v, idx) {
    return _c('i-option', {
      key: idx,
      attrs: {
        "value": idx
  ...
  }, [_vm._v("新增商品")]), _vm._v(" "), (_vm.shopList && _vm.shopList.length) ? _c('i-button', {
    attrs: {
      "icon": "md-copy"
    },
    on: {
      "click": _vm.openProductCopyDlg
    }
  }, [_vm._v("复制商品到其它门店")]) : _vm._e(), _vm._v(" "), (_vm.g.data.user.isShopOwner) ? _c('i-button', {
    on: {
    ...
    return _c('tr', [(_vm.editable) ? _c('td', {
      staticClass: "ckbox",
      on: {
        "click": function($event) {
          return _vm.checkProduct(p.productId)
        }
      }
    }, [(!_vm.selectProdIdList.contains(p.productId)) ? _c('i', {
      staticClass: "iconfont icon-checkbox-empty ckbox-0"
    }) : _c('i', {
      staticClass: "iconfont icon-checkbox-checked ckbox-1"
    ...
    }, [_vm._v(_vm._s(p.orderNo))])]), _vm._v(" "), _c('td', [_vm._v(_vm._s(p.cateName))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(p.productName)), (p.cardName) ? _c('span', {
      staticClass: "card"
    }, [_vm._v(_vm._s(p.cardName))]) : _vm._e()]), _vm._v(" "), _c('td', [(p.status === 0) ? [_vm._v("上架")] : _vm._e(), _vm._v(" "), (p.status === 1) ? _c('span', {
      staticClass: "gray"
    ...
    }, [(p.limitBuyTimes) ? _c('li', [_vm._v(_vm._s(p.limitBuyTimes) + " 次")]) : _vm._e(), _vm._v(" "), (p.limitBuyNum) ? _c('li', [_vm._v(_vm._s(p.limitBuyNum) + " 件/次")]) : _vm._e()])]), _vm._v(" "), _c('td', [_vm._v(_vm._s(p.stock))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(p.salesCnt))]), _vm._v(" "), _c('td', [_c('a', {
      on: {
        "click": function($event) {
      ...
      staticClass: "spt"
    }, [_vm._v("|")]), _vm._v(" "), _c('Dropdown', [_c('a', {
      attrs: {
      ...
      on: {
        "click": function($event) {
          return _vm.openProductLinkDlg(p)
        }
      }
    }, [_vm._v("商品二维码")])]), _vm._v(" "), _c('DropdownItem', [_c('div', {
      on: {
        "click": function($event) {
          return _vm.viewProductLog(p.productId, p.productName)
        }
      }
    }, [_vm._v("操作日志")])]), _vm._v(" "), _c('DropdownItem', [_c('div', {
      on: {
        "click": function($event) {
          return _vm.handleAltProductStock(p)
        }
      }
    }, [_vm._v("入库")])]), _vm._v(" "), (p.status === 0) ? _c('DropdownItem', [_c('div', {
      on: {
        "click": function($event) {
          return _vm.updateProductStatus(p.productId, 1)
        }
      }
    }, [_vm._v("下架")])]) : _vm._e(), _vm._v(" "), (p.status === 1) ? _c('DropdownItem', [_c('div', {
      on: {
        "click": function($event) {
          return _vm.updateProductStatus(p.productId, 0)
        }
      }
    }, [_vm._v("上架")])]) : _vm._e(), _vm._v(" "), _c('DropdownItem', [_c('div', {
      on: {
        "click": function($event) {
    ...
    staticClass: "no-data"
  }, [_vm._v("\n\t\t\t\t暂无数据\n\t\t\t")]) : _vm._e()]), _vm._v(" "), (!_vm.g.data.shop.hasWxPay) ? _c('div', {
    staticClass: "warning"
  }, [_c('i', {
    staticClass: "iconfont icon-alert2 big"
    ...
    staticClass: "note"
  }, [_vm._m(0), _vm._v(" "), _c('ol', [_vm._m(1), _vm._v(" "), _c('li', [_vm._v("微商城融合了积分商城和在线购物商城；")]), _vm._v(" "), _c('li', [_vm._v("微商城的商品可以是会员卡或者实物, 商品价格可以设置会员积分或金额；")]), _vm._v(" "), _c('li', [_vm._v("微商城中的商品是会员在线上（手机端）购买，记账中的商品是线下购买，两者无联系；")]), _vm._v(" "), _c('li', [_vm._v("会员在线支付统一使用微信支付，需要你们先开通微信在线支付功能（请参考 "), _c('a', {
    on: {
      "click": function($event) {
        return _vm.$router.push('/online-pay-apply')
      }
    }
  }, [_vm._v("在线支付申请")]), _vm._v("）")]), _vm._v(" "), _c('li', [_vm._v("微商城二维码：\n\t\t\t\t\t"), _c('div', [_c('img', {
    staticStyle: {
      "width": "200px"
    },
    attrs: {
      "src": _vm.g.Conf.QR_CODE_URL + encodeURIComponent(((_vm.g.Conf.MOBILE_CLIENT_URL) + "#/mall/product-list?encSid=" + (encodeURIComponent(_vm.g.data.shop.encSid))))
    }
  })])])])])]), _vm._v(" "), _c('modal', {
    attrs: {
    ...
    model: {
      value: (_vm.prodCopyDlg),
      callback: function($$v) {
        _vm.prodCopyDlg = $$v
      },
      expression: "prodCopyDlg"
    }
  }, [_vm._v("\n\t\t请选择复制商品到哪些门店：\n\t\t"), _c('CheckboxGroup', {
    staticClass: "shop-list",
    model: {
      value: (_vm.selectEncSidList),
      callback: function($$v) {
        _vm.selectEncSidList = $$v
      },
      expression: "selectEncSidList"
    }
  }, _vm._l((_vm.shopList), function(s) {
    return (_vm.g.data.shop.encSid !== s.encSid) ? _c('Checkbox', {
      key: s.encSid,
      attrs: {
        "label": s.encSid
      }
    ...
    on: {
      "click": _vm.copyProdctToShops
    }
  }, [_vm._v("提交")]), _vm._v(" "), _c('i-button', {
    on: {
      "click": function($event) {
        _vm.prodCopyDlg = false
      }
    }
  }, [_vm._v("关闭")])], 1), _vm._v(" "), _c('div', {
    attrs: {
    ...
    model: {
      value: (_vm.productLinkAndQRCodeDlg),
      callback: function($$v) {
        _vm.productLinkAndQRCodeDlg = $$v
      },
      expression: "productLinkAndQRCodeDlg"
    }
  }, [(_vm.productLinkAndQRCodeDlg && _vm.selectProduct) ? [_c('table', {
    staticClass: "prdlink"
  }, [_c('tr', [_c('td', {
    attrs: {
      "width": "110"
    }
  }, [_vm._v("商品名称：")]), _c('td', [_c('h3', [_vm._v(_vm._s(_vm.selectProduct.productName))])])]), _vm._v(" "), _c('tr', [_c('td', [_vm._v("商品二维码：")]), _vm._v(" "), _c('td', [_c('div', {
    staticClass: "qr-box"
  }, [(!_vm.selectProductUrl) ? _c('div', {
    ref: "myQrCode",
    staticClass: "qr-code-img"
  }, [(_vm.selectProduct.shareLinkUrl) ? _c('qrcode-vue', {
    attrs: {
      "value": _vm.selectProduct.shareLinkUrl,
      "size": 160,
      "level": "H"
    }
  }) : _vm._e(), _vm._v(" "), _c('div', {
    staticClass: "qr-staff-name"
  }, [_vm._v(_vm._s(_vm.selectProduct.productName))])], 1) : _c('img', {
    staticClass: "qr-code-img-url",
    attrs: {
      "src": _vm.selectProductUrl,
      "alt": ""
    }
  })])])]), _vm._v(" "), _c('tr', [_c('td', [_vm._v("商品链接：")]), _vm._v(" "), _c('td', [_c('div', {
    staticClass: "link-value",
    on: {
      "click": function($event) {
        return _vm.g.Util.copyText($event, _vm.selectProduct.shareLinkUrl)
      }
    }
  }, [_c('div', {
    staticClass: "link"
  }, [_vm._v(_vm._s(_vm.selectProduct.shareLinkUrl))]), _vm._v(" "), _c('span', {
    ...
    on: {
      "click": function($event) {
        _vm.productLinkAndQRCodeDlg = false
      }
    }
  }, [_vm._v("关闭")])], 1)], 2)], 1)
},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('shop-nav', {
    ...
    model: {
      value: (_vm.query.productName),
      callback: function($$v) {
        _vm.$set(_vm.query, "productName", (typeof $$v === 'string' ? $$v.trim() : $$v))
      },
      expression: "query.productName"
    }
  })], 1), _vm._v(" "), _c('li', {
  ...
  }, [_vm._v("操作")])]), _vm._v(" "), _vm._l((_vm.orderList), function(o) {
    return _c('tr', [_c('td', [_vm._v(_vm._s(o.createTime))]), _vm._v(" "), _c('td', {
      ...
      return _c('li', [_vm._v("\n\t\t\t\t\t\t\t\t\t\t\t\t" + _vm._s(p.productName) + " × " + _vm._s(p.unitNum) + "\n\t\t\t\t\t\t\t\t\t\t\t\t"), (p.consigneeTime) ? _c('span', {
        staticClass: "gray"
      }, [_vm._v("（已领）")]) : _vm._e(), _vm._v(" "), (p.refundStatus === 9) ? _c('span', {
        staticClass: "gray"
      }, [_vm._v("（已退）")]) : _vm._e()])
  ...
  }, [_c('OrderRefund')], 1)], 1)], 1), _vm._v(" "), _c('modal', {
    attrs: {
      "title": "会员基本信息",
      "width": "800",
      "styles": {
  ...
  }, [_vm._v("订单时间")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.selectOrder.createTime))])]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("订单状态")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Dict.OrderStatus[_vm.selectOrder.status]))])]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("会员")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.selectOrder.memberName))])]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("支付时间")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.selectOrder.transTime))])]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("支付款")]), _vm._v(" "), _c('td', [(_vm.selectOrder.paymentPoints) ? [_vm._v(_vm._s(_vm.selectOrder.paymentPoints) + " 积分")] : _vm._e(), _vm._v(" "), (_vm.selectOrder.paymentPoints && _vm.selectOrder.paymentAmount) ? _c('b', {
    staticClass: "big"
    ...
    return _c('li', [_vm._v("\n\t\t\t\t\t\t\t\t\t※ " + _vm._s(p.productName) + " × " + _vm._s(p.unitNum) + "\n\t\t\t\t\t\t\t\t\t"), _c('span', {
      ...
      staticClass: "spt"
    }, [_vm._v("/")]), _vm._v("  \n                    "), (p.consigneeTime) ? [_vm._v(" " + _vm._s(p.consigneeTime) + " 领取")] : (_vm.selectOrder.status === 9) ? _c('a', {
      on: {
        "click": function($event) {
          return _vm.confirmProductReceipt(idx)
        }
      }
    }, [_vm._v("【领取】")]) : _vm._e(), _vm._v(" "), (p.refundStatus === 9) ? _c('span', {
      staticClass: "gray"
    }, [_vm._v("（已退）")]) : _vm._e(), _vm._v(" "), (_vm.isOpenShopCoupon && p.couponLogIds) ? _c('a', {
      staticClass: "r",
      on: {
        "click": function($event) {
          return _vm.quyeryCouponLogList(p)
        }
      }
    }, [_vm._v("【优惠券使用记录】")]) : _vm._e()], 2)])
  }), 0)])])])]) : _vm._e()]), _vm._v(" "), _c('modal', {
    attrs: {
      "title": ("商品【" + _vm.productName + "】-优惠券使用记录"),
      "width": "700",
      "styles": {
        top: '10px'
      },
    ...
    model: {
      value: (_vm.useCouponDlg),
      callback: function($$v) {
        _vm.useCouponDlg = $$v
      },
      expression: "useCouponDlg"
    }
  }, [_c('div', {
    staticClass: "mytbl use-coupon-dlg"
  }, _vm._l((_vm.couponLogList), function(l) {
    return _c('table', {
      key: l.logId
    }, [_c('tr', [_c('th', {
      attrs: {
    ...
    }, [_vm._v("优惠券名称")]), _vm._v(" "), _c('td', [_vm._v("【" + _vm._s(_vm.g.Dict.CouponTypes[l.couponType]) + "】" + _vm._s(l.title))])]), _vm._v(" "), _c('tr', [_c('th', {
      attrs: {
        "width": "100"
      }
    }, [_vm._v("券面额")]), _vm._v(" "), _c('td', [(l.couponType === 1) ? _c('span', [_c('b', [_vm._v(_vm._s(l.value))]), _vm._v(" 元")]) : _vm._e(), _vm._v(" "), (l.couponType === 2) ? _c('span', [_c('b', [_vm._v(_vm._s(l.value))]), _vm._v(" 折")]) : _vm._e(), _vm._v(" "), (l.couponType === 3) ? _c('span', [_vm._v("满 "), _c('b', [_vm._v(_vm._s(l.preValue))]), _vm._v(" 减 "), _c('b', [_vm._v(_vm._s(l.value))]), _vm._v(" 元")]) : _vm._e()])]), _vm._v(" "), _c('tr', [_c('th', {
      attrs: {
        "width": "100"
      }
    }, [_vm._v("领取时间")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.createTime))])]), _vm._v(" "), _c('tr', [_c('th', {
      attrs: {
        "width": "100"
      }
    }, [_vm._v("使用时间")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.usedTime))])])])
  }), 0)])], 1)
},staticRenderFns: []}
