...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__pages_member_member_group_lesson_opt_vue__ = __webpack_require__(1165);
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    loadData() {
      ...
      $.Req.service($.SvName.SHOP_ARG_QUERY, args, ret => {
        let hasCourseConf = null;
        for (let t of ret.argList) {
          if (t.typeId === 2003) {
            this.setCoachConf(t.typeValue);
          } else if (t.typeId === 2212 || t.typeId === 2214) {
            if (t.typeValue) this.bgColorType = parseInt(t.typeValue);
          } else if (t.typeId === 2213 || t.typeId === 2215) {
            hasCourseConf = true;
            this.queryCourseList(t.typeValue);
          }
        }
        if (!hasCourseConf) {
          this.queryCourseList(null);
        }
      });
    },

    // 修改
    getColorMap(typeValue) {
      let argVal = typeValue;
      let colorMap = {};
      let textColorMap = {};
      if (argVal) {
        ...
        for (let item of items) {
          ...
          textColorMap[staffId] = arr[2] || this.defaultTextColor;
        }
      }
    ...
    // 修改
    setCoachConf(typeValue) {
      let { colorMap, textColorMap } = this.getColorMap(typeValue);
      $.Req.queryCoachBase(null, coachList => {
        this.coachList = coachList;
        for (let c of coachList) {
          if (colorMap[c.staffId]) {
            this.coachColorMap[c.staffId] = colorMap[c.staffId];
          } else {
            this.coachColorMap[c.staffId] = this.defaultColor;
          }
          if (textColorMap[c.staffId]) {
            this.coachTextColorMap[c.staffId] = textColorMap[c.staffId];
          } else {
            this.coachTextColorMap[c.staffId] = this.defaultTextColor;
          }
        }
      });
    },
    ...
    // 修改
    setCourseColorMap(colorMap, textColorMap) {
      for (let c of this.courseList) {
        if (colorMap[c.courseId]) {
          this.courseColorMap[c.courseId] = colorMap[c.courseId];
        } else {
          this.courseColorMap[c.courseId] = this.defaultColor;
        }
        if (textColorMap[c.courseId]) {
          this.courseTextColorMap[c.courseId] = textColorMap[c.courseId];
        } else {
          this.courseTextColorMap[c.courseId] = this.defaultTextColor;
        }
      }
    },

    queryCourseList(typeValue) {
      let { colorMap, textColorMap } = this.getColorMap(typeValue);
      if (this.lessonType === 1) {
        $.Req.queryGroupCourseBase(data => {
          this.courseList = data;
          this.setCourseColorMap(colorMap, textColorMap);
        });
      } else {
    ...
    selectStaff(staffId) {
      ...
      // 新增
      this.textColor = "#" + this.coachTextColorMap[staffId];
      this.bgColor = "#" + this.coachColorMap[staffId];
    },

    selectCourse(courseId) {
      this.selectCourseId = courseId;
      // 新增
      this.textColor = "#" + this.courseTextColorMap[courseId];
      this.bgColor = "#" + this.courseColorMap[courseId];
    },
    ...
    setColor(color) {
      ...
      if (this.bgColorType === 1) {
        ...
        this.courseColorMap[this.selectCourseId] = color;
      }
      this.bgColor = "#" + color;
      this.$forceUpdate();
    },
    ...
    saveColorConf() {
      let items = [];
      let typeId = this.lessonType === 1 ? 2212 : 2214;
      $.Req.updateShopArg(typeId, this.bgColorType, false);
      if (this.bgColorType === 1) {
        for (let staffId in this.coachColorMap) {
          items.push("{0}:{1}:{2}".format(staffId, this.coachColorMap[staffId], this.coachTextColorMap[staffId]));
        }
        let args = {
          typeId: $.Dict.ShopArg.COACH_LESSON_BGCOLOR,
          typeValue: items.join(",")
        };
        $.Req.service($.SvName.SHOP_ARG_UPDATE, args, ret => {
          $.Msg.success($.Lang.OPT_SUCCESS);
          this.afterSubmit(this.bgColorType);
        });
      } else if (this.bgColorType === 2) {
        for (let courseId in this.courseColorMap) {
          items.push("{0}:{1}:{2}".format(courseId, this.courseColorMap[courseId], this.courseTextColorMap[courseId]));
        }
        let args = {
          typeId: this.lessonType === 1 ? 2213 : 2215,
          typeValue: items.join(",")
        };
        $.Req.service($.SvName.SHOP_ARG_UPDATE, args, () => {
          this.afterSubmit(this.bgColorType);
        }, true);
      }
    },
    ...
    changeBgColor() {
      if (this.bgColorType == 1) {
        this.coachColorMap[this.selectCoachId] = this.bgColor.replace("#", "") || this.defaultColor;
      } else {
        this.courseColorMap[this.selectCourseId] = this.bgColor.replace("#", "") || this.defaultColor;
      }
    },

    changeFontColor() {
      if (this.bgColorType == 1) {
        this.coachTextColorMap[this.selectCoachId] = this.textColor.replace("#", "") || this.defaultTextColor;
      } else {
        this.courseTextColorMap[this.selectCourseId] = this.textColor.replace("#", "") || this.defaultTextColor;
      }
    }
  }
});
...
/* harmony default export */ __webpack_exports__["default"] = ({
	...
	methods: {
		...
		queryLogList() {
			$.Req.service($.SvName.COURSE_SCHEDULE_CHANGE_LOG_QUERY, { csId: this.csId }, ret => {
				let memberIdList = [];
				for (let l of ret.logList) {
					if (l.classMembers) {
						for (let mid of l.classMembers.split(',')) {
							mid = parseInt(mid);
							if (!memberIdList.contains(mid)) memberIdList.push(mid);
						}
					}
				}
				if (memberIdList.length) {
					...
					$.Req.service($.SvName.MEMBER_BASE_LIST_QUERY, args, ret => {
						...
						this.mlMemberList = [];
						for (let m of ret.memberBaseList) {
							this.memberNameMap[m.memberId] = m.name;
						}
					});
				}
		...
		formatWeekTimes(val) {
			...
			for (let item of val.split(',')) {
				...
				timeArr.push('{0} ~ {1}'.format($.Util.formatTime(times[0]), $.Util.formatTime(times[1])));
			}
			return timeArr.join('，');
		},

		formatCardPaymentConf(conf) {
			if (!conf) return;
			...
			for (let item of conf.split(';')) {
				...
				let card = this.cardMap[cardId];
				let cardInfo = card.cardName;
				if (arr.length > 1 && card.cardType > 1) {
					cardInfo += "：" + arr[1] + $.Dict.CardUnitName[card.cardType];
				}
				confArr.push(cardInfo);
			}

			return confArr.join('\n');
		},

		formatMemberNames(memberIds) {
			if (!memberIds) return;

			let names = [];
			for (let id of memberIds.split(',')) {
				id = parseInt(id);
				names.push(this.memberNameMap[id]);
			}
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__pages_member_member_select_vue__ = __webpack_require__(1123);
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__pages_course_cust_private_lesson_booking_vue__ = __webpack_require__(2246);
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__ = __webpack_require__(1122);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__lesson_review_list__ = __webpack_require__(1148);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__pages_member_member_select_vue__ = __webpack_require__(1123);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__tenant_tenant_mp_auth__ = __webpack_require__(875);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__group_course_schedule__ = __webpack_require__(878);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__group_course_schedule___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_5__group_course_schedule__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__course_schedule_change_log_vue__ = __webpack_require__(2245);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__course_schedule_change_log_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_6__course_schedule_change_log_vue__);
//
//
//
//
...
/* harmony default export */ __webpack_exports__["default"] = ({
	components: { GroupCourseSchedule: __WEBPACK_IMPORTED_MODULE_5__group_course_schedule___default.a, TenantMpAuth: __WEBPACK_IMPORTED_MODULE_4__tenant_tenant_mp_auth___default.a, PageHeader: __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue___default.a, Help: __WEBPACK_IMPORTED_MODULE_1__components_help___default.a, MemberSelect: __WEBPACK_IMPORTED_MODULE_2__pages_member_member_select_vue___default.a, MemberBaseInfo: __WEBPACK_IMPORTED_MODULE_3__pages_member_member_base_info_vue___default.a, CourseScheduleChangeLog: __WEBPACK_IMPORTED_MODULE_6__course_schedule_change_log_vue___default.a },
	data() {
		return {
			g: $,
			editable: $.Util.hasRoleFunc($.Dict.RoleFunc.COURSE_EDIT.value),
	...
	methods: {
		loadCs() {
			...
			$.Req.service($.SvName.COURSE_SCHEDULE_QUERY, args, ret => {
				...
				if (this.cs.limitMemberTags) {
					for (let tagId of this.cs.limitMemberTags.split(',')) {
						this.includeMemberTagIdList.push(parseInt(tagId));
					}
				}
			}, true);
		},
		...
		handleSubmit() {
			this.$refs.cs.validate(valid => {
				...
				if (this.cs.isNeedBooking) {
					if (!this.cs.maxStudents || this.cs.maxStudents < 1) {
						$.Dlg.error('请设置有效的最多预约人数。');
						return false;
					}
				}
				...
				if (!this.cs.csId) {
					if (this.cs.beginDate.compareDatePart(new Date()) < 0) {
						$.Dlg.error('课程开始日期【{0}】不能小于今日'.format($.Util.formatDate(this.cs.beginDate)));
						return false;
					}
				}
				if (this.cs.beginDate.compareDatePart(this.cs.endDate) > 0) {
					$.Dlg.error('课程结束日期【{0}】不能小于开始日期【{1}】'.format($.Util.formatDate(this.cs.beginDate), $.Util.formatDate(this.cs.endDate)));
					return false;
				}
				...
				this.cs.limitMemberTags = $.Util.isNotEmpty(this.includeMemberTagIdList) ? this.includeMemberTagIdList.join(',') : null;
		...
		checkAllCards(val) {
			...
			for (let c of this.cardList) {
				if (!this.cardClassList.contains(c.cardClass)) {
					continue;
				}
				if (c.status !== 0 && !this.showPausedCards) {
					continue;
				}
		...
		afterSelectMember(findedMember) {
			...
			if (this.classMembers.length > 150) {
				$.Dlg.error('最多只支持添加150个固定学员');
				return;
			}
			for (let m of this.classMembers) {
			...
			this.classMembers.push({ memberId: findedMember.memberId, name: findedMember.name });
		...
		viewLessonsByCs() {
			this.lessonsDlg = true;
			this.queryLessonsByCs(null);
		},

		queryLessonsByCs(page = null) {
			if (this.lessonQuery.beginDate && !this.lessonQuery.endDate || !this.lessonQuery.beginDate && this.lessonQuery.endDate) {
				$.Msg.error('请输入合法的日期格式');
				return;
			}
			let args = {
				csId: this.csId, page: page, needPaging: true,
				beginDate: this.lessonQuery.beginDate,
				endDate: this.lessonQuery.endDate
			};
			this.lessonQuery.page = page;
			$.Req.service($.SvName.LESSON_QUERY, args, ret => {
				this.lessonList = ret.lessonList;
				if (!page) {
					this.lessonCount = ret.count;
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__lesson_review_list_vue__ = __webpack_require__(1148);
...
/* harmony default export */ __webpack_exports__["default"] = ({
	components: { LessonReviewList: __WEBPACK_IMPORTED_MODULE_0__lesson_review_list_vue___default.a, MemberSelect: __WEBPACK_IMPORTED_MODULE_1__pages_member_member_select_vue___default.a, Help: __WEBPACK_IMPORTED_MODULE_2__components_help___default.a },
	data() {
		return {
			g: $,
			...
			selectCs: null,
			updateCsGcTypesDlg: null,
			selectGcType: null,
			isBatchAppendCards: null,
	...
	methods: {
		...
		updateCsDates() {
			if (!this.quickEditTime) {
				...
				let args = {
					csId: cs.csId,
					actionType: 'U',
					beginDate: Date.new(this.selectCsDates[0]),
					endDate: Date.new(this.selectCsDates[1])
				};
				$.Req.service($.SvName.COURSE_SCHEDULE_SAVE, args, ret => {
					cs.beginDate = args.beginDate;
					cs.endDate = args.endDate;
				...
				$.Req.service($.SvName.COURSE_SCHEDULE_SAVE, args, ret => {
					$.Msg.success($.Lang.OPT_SUCCESS);
					let cs = this.courseScheduleList[this.selectCsIdx];
					cs.beginDate = this.cs.beginDate;
					cs.endDate = this.cs.endDate;
					this.$set(this.courseScheduleList, this.selectCsIdx, cs);
					this.closeEditTimeDlg();
				}, true);
			}
		},
		...
		openCsCardAppendDlg() {
			if (!this.checkCsSelection()) return;
		...
		submitAppendCardPaymentToCsList() {
			let args = {};
			let cardType = null;
			if (this.isBatchAppendCards) {
				if ($.Util.isEmptyArray(this.selectCardIdList)) {
					$.Msg.error('请选择会员卡');
					return;
				}
				...
				args = { cardId: this.selectCardId };
				cardType = this.selectCard.cardType;
			}
			if (cardType == 1) {
		...
		openUpateGcTypeDlg() {
			...
			this.updateCsGcTypesDlg = true;
		},

		updateCsGcType() {
			if (!confirm('您确定批量更新课程类型吗？')) return;

			let args = { gcType: this.selectGcType };
			args.csIdList = [];
			for (let i = 0, len = this.psCkList.length; i < len && i < this.courseScheduleList.length; i++) {
				if (this.psCkList[i]) {
					args.csIdList.push(this.courseScheduleList[i].csId);
			...
			$.Req.service($.SvName.COURSE_SCHEDULE_GC_TYPE_UPDATE, args, ret => {
				$.Msg.success($.Lang.OPT_SUCCESS);
				this.updateCsGcTypesDlg = false;
				let i = 0;
				for (let cs of this.courseScheduleList) {
					if (args.csIdList.contains(cs.csId)) {
						cs.gcType = this.selectGcType;
						this.$set(this.courseScheduleList, i, cs);
					}
					i++;
				}
			}, true);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__ = __webpack_require__(1122);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__lesson_review_list_vue__ = __webpack_require__(1148);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__ = __webpack_require__(1122);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__pages_course_group_lesson_records_vue__ = __webpack_require__(1164);
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__pages_course_coach_lesson_bgcolor_conf_vue__ = __webpack_require__(1223);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__lesson_img_list__ = __webpack_require__(2249);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__lesson_coach_comment_list__ = __webpack_require__(2248);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__lesson_review_list__ = __webpack_require__(1148);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__pages_course_group_lesson_records_vue__ = __webpack_require__(1164);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__tenant_tenant_mp_auth__ = __webpack_require__(875);
...
/* harmony default export */ __webpack_exports__["default"] = ({
	components: {
		...
		CoachLessonBgcolorConf: __WEBPACK_IMPORTED_MODULE_0__pages_course_coach_lesson_bgcolor_conf_vue___default.a, LessonImgList: __WEBPACK_IMPORTED_MODULE_1__lesson_img_list___default.a, LessonCoachCommentList: __WEBPACK_IMPORTED_MODULE_2__lesson_coach_comment_list___default.a, LessonReviewList: __WEBPACK_IMPORTED_MODULE_3__lesson_review_list___default.a, GroupLessonRecords: __WEBPACK_IMPORTED_MODULE_4__pages_course_group_lesson_records_vue___default.a },
	data() {
		return {
			g: $,
			courseEditable: $.Util.hasRoleFunc($.Dict.RoleFunc.COURSE_EDIT.value),
			...
			lessonPage: null,
			isCombineLessonDate: true,
			exportDlg: null,
			exportFileUrl: null,
			bgColorType: 1,
			defaultBgColor: "777777",
			defaultTextColor: "FFFFFF",
	...
	watch: {
		...
		["newLesson.coachId"](newVal) {
			if (!newVal) return;
			this.queryCourseScheduleList();
		}
	},
	...
	methods: {
		...
		queryLessonList(page = null, isExport = null) {
			...
			$.Req.service($.SvName.LESSON_QUERY, args, ret => {
				...
				for (let l of ret.lessonList) {
					...
					if (l.bookings && l.maxStudents) {
						bookingRatesCnt++;
						bookingRatesSum += l.bookings / l.maxStudents;
					}
				}
				this.sum.bookings = sumOfbookings;
				this.sum.checkins = sumOfCheckins;
				this.sum.students = sumOfStudents;
				this.sum.avgBookingRates = bookingRatesCnt ? $.Util.round(100 * bookingRatesSum / bookingRatesCnt) : 0;
				if (page === null) {
					this.lessonCount = ret.count;
				} else {
					window.scrollTo(0, 0);
				...
				if (this.query.dateType === 'week') {
					for (let l of ret.lessonList) {
						if (this.bgColorType === 1) {
							if (!this.bgColorMap[l.coachId]) this.bgColorMap[l.coachId] = this.defaultBgColor;
							if (!this.textColorMap[l.coachId]) this.textColorMap[l.coachId] = this.defaultTextColor;
						} else {
							if (!this.bgColorMap[l.courseId]) this.bgColorMap[l.courseId] = this.defaultBgColor;
							if (!this.textColorMap[l.courseId]) this.textColorMap[l.courseId] = this.defaultTextColor;
						}
					}
		...
		afterSubmitBgColor(bgColorType) {
		...
		queryShopArgs(callbck = null) {
			let args = {
				typeIdList: [$.Dict.ShopArg.COACH_LESSON_BGCOLOR, 2087, 2212, 2213]
			};
			$.Req.service($.SvName.SHOP_ARG_QUERY, args, ret => {
				let coachColorConf = null,
				    courseColorConf = null;
				for (let t of ret.argList) {
					if (t.typeId === 2212) {
						if (t.typeValue) this.bgColorType = parseInt(t.typeValue);
					} else if (t.typeId === $.Dict.ShopArg.COACH_LESSON_BGCOLOR) {
						coachColorConf = t.typeValue;
					} else if (t.typeId === 2087) {
						this.enableGlCheckout = t.typeValue === 'true';
					} else if (t.typeId === 2213) {
						courseColorConf = t.typeValue;
					}
				}
				this.parseBgColor(this.bgColorType === 1 ? coachColorConf : courseColorConf);
		...
		queryCoachBgColorConf(callbck) {
			let args = { typeId: this.bgColorType === 1 ? $.Dict.ShopArg.COACH_LESSON_BGCOLOR : 2213 };
			$.Req.service($.SvName.SHOP_ARG_QUERY, args, ret => {
				this.parseBgColor(ret.value);
				if (callbck) callbck();
			});
		},
		...
		parseBgColor(argVal) {
			...
			if (argVal) {
				...
				for (let item of items) {
					...
					let staffId = parseInt(arr[0]);
					colorMap[staffId] = arr[1];
					if (arr.length > 2) textColorMap[staffId] = arr[2];
				}
			}
			this.bgColorMap = colorMap;
		...
		exportLessonListTable() {
			$.Util.exportToExcel('groupLesonListTbl', '团课课表');
		},
		...
		createLesson() {
			...
			let args = {
				csId: this.newLesson.csId,
				lessonTitle: this.newLesson.lessonTitle,
				coachId: this.newLesson.coachId,
				lessonDate: this.newLesson.lessonDate,
				beginTime: $.Util.getTimeNum(this.newLesson.lessonTimes[0]),
				endTime: $.Util.getTimeNum(this.newLesson.lessonTimes[1]),
		...
		clickLessonTable(bt, w) {
			if (this.lessonMap[bt][w]) return;
			if (!confirm('是否新增一节课？')) return;
			const [a, b] = bt.split(':');
			let ba = a == 12 ? ['00', b] : [+a + 1, b];
			let lessonDate = this.weekDates[w - 1 >= 0 ? w - 1 : 6];
			let lessonTimes = [bt, ba.join(':')];
			// if (Date.now() >= new Date(lessonDate).setHours(a, b, 0)) { // 判断当前时间是否符合新增课程要求
			// 	lessonDate = new Date()
			// 	lessonTimes = []
			// }
			this.newLesson = {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__gl_booking_blacklist_vue__ = __webpack_require__(2247);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__pl_checkin_conf_vue__ = __webpack_require__(2250);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__pages_member_member_base_info_vue__ = __webpack_require__(191);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__pages_member_member_base_info_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_3__pages_member_member_base_info_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__tenant_tenant_stat_member__ = __webpack_require__(876);
...
/* harmony default export */ __webpack_exports__["default"] = ({
	...
	data() {
		...
		return {
			...
			cardClassList: [2],
			moreExtrCardConfList: [],
			needExtraCardCheck: true,
			displayView: 2, // 1-精简，2-专业
			showPausedCards: null,
			monthGradeFeeList: [[null, null, null]],
			disablePsMember: null,
			shopTagList: null,
			includeMemberTagIdList: [],
			excludeMemberTagIdList: []
		};
	},

	created() {
		if (this.$route.params.psId) {
			this.cardClassList = [0, 1, 2];
		}
		$.Util.addRequiredFieldsRules(this.csFormRule, this.requiredFields);
		let displayViewCookieVal = $.Util.cookie.get($.Conf.CookieKeys.PS_DISPLAY_VIEW);
		if (displayViewCookieVal) {
			this.displayView = parseInt(displayViewCookieVal);
		}
		$.Req.queryPrivateCourseBase(data => {
			this.courseList = data;
		});
	...
	methods: {
		queryPs() {
			...
			$.Req.service($.SvName.PRIVATE_COURSE_SCHEDULE_QUERY, args, ret => {
				...
				if ($.Util.isEmpty(this.ps.minCheckinsForEnd)) this.ps.minCheckinsForEnd = 1;
				if (this.ps.ignoreExtraCardCheck) this.needExtraCardCheck = false;
				this.moreExtrCardConfList = this.getMoreExtrCardConfList(this.ps.moreExtraCardsConf);
				this.coachFeeConfList = ret.coachFeeConfList;
				this.setWeekTimes();
				this.setCardPayments();
				...
				if (this.ps.coachLessonFeeType === 4) {
					this.createMonthGradeFeeList();
				}
				if (this.ps.includeMemberTags) {
					for (let tagId of this.ps.includeMemberTags.split(',')) {
						this.includeMemberTagIdList.push(parseInt(tagId));
					}
				}
				if (this.ps.excludeMemberTags) {
					for (let tagId of this.ps.excludeMemberTags.split(',')) {
						this.excludeMemberTagIdList.push(parseInt(tagId));
					}
				}
			});
		},
		...
		handleSubmit() {
			this.$refs.ps.validate(valid => {
				...
				let warning = this.checkCourseName();
				if (warning) if (!confirm(warning)) return;

				if (this.ps.beginDate && !this.ps.endDate || !this.ps.beginDate && this.ps.endDate) {
				...
				if (this.ps.coachLessonFeeType === 4) {
					// 包月梯度课时费
					let conf = this.genMonthGradeFeeConf();
					if (!conf) return;
					this.ps.coachLessonFeeConf = conf;
				}
				...
				if (this.ps.enableMoreStudents && !this.member.memberId) {
					...
					if (this.ps.minCheckinsForEnd && this.ps.minCheckinsForEnd < 1) {
						$.Dlg.error('最少签到人数必须大于0');
						return;
					}
				}
				...
				$.Dlg.confirm(msg, yes => {
					...
					this.ps.moreExtraCardsConf = this.genMoreExtraCardsConf();
					if (this.needExtraCardCheck === false) this.ps.ignoreExtraCardCheck = true;else this.ps.ignoreExtraCardCheck = null;
					this.ps.includeMemberTags = $.Util.isNotEmpty(this.includeMemberTagIdList) ? this.includeMemberTagIdList.join(',') : null;
					this.ps.excludeMemberTags = $.Util.isNotEmpty(this.excludeMemberTagIdList) ? this.excludeMemberTagIdList.join(',') : null;
					if (!this.ps.memberId && $.Util.isEmptyArray(this.limitMembers)) {
						this.ps.limitTimes = null;
					}
		...
		getMoreExtrCardConfList(conf) {
			if (!conf) return [];

			let confList = [];
			for (let i of conf.split(',')) {
				let items = i.split(':');
				confList.push({ cardId: parseInt(items[0]), cardPayment: parseFloat(items[1]) });
			}

			return confList;
		},

		genMoreExtraCardsConf() {
			let confList = [];
			for (let c of this.moreExtrCardConfList) {
				if (c.cardId && $.Util.isNotEmpty(c.cardPayment)) {
					confList.push(c.cardId + ':' + c.cardPayment);
				}
			}
		...
		genWeekTimes() {
			...
			for (let key in this.timeValList) {
				...
				if (Array.isArray(val) && val[0]) {
					...
					weekTimes[weekIdx].push(beginTime + '-' + endTime);
					if (beginTime < 400 || beginTime > endTime) {
						throw '上课时间【{0} ~ {1}】设置不合理，请重新设置'.format(beginDateTime, endDateTime);
					}
					if (endTime - beginTime > 400) {
						throw '上课时长【{0} ~ {1}】不能超过4小时，请重新设置'.format(beginDateTime, endDateTime);
					}
				}
			}
		...
		queryMemberCardsForPl(callback = null) {
			...
			$.Req.service($.SvName.MEMBER_CARD_QUERY, args, (ret, systime) => {
				...
				this.canLimitMcIds = this.memberCardList.length > 0; // ($.Util.countAttrs(this.memberCardIdMap) !== this.memberCardList.length);
				this.psMcIdList = [];
				if (this.ps.mcIds) {
			...
			$.Req.service($.SvName.MEMBER_BASE_QUERY, { memberId: memberId }, ret => {
				if (ret.member) {
					...
					if (this.$route.params.lessonCnt) {
						...
						if (val > 0) {
							if (confirm('需要填写默认的排课次数吗？')) {
								this.ps.limitTimes = val;
							}
						}
					}
					if ($.Util.isNotEmpty(this.$route.params.lessonFee)) {
		...
		addMoreExtrCardConf() {
			this.moreExtrCardConfList.push({
				cardId: null, cardPayment: null
			});
		},

		changeDisplayView(val) {
			$.Util.cookie.set($.Conf.CookieKeys.PS_DISPLAY_VIEW, val, 365);
		},

		setMoreLessonMembers(memberCnt) {
			if (this.displayView !== 2) {
				this.displayView = 2;
				this.changeDisplayView(2);
			}
			if (!this.ps.enableMoreStudents) {
				this.$set(this.ps, 'enableMoreStudents', true);
			}
			this.$set(this.ps, 'maxStudents', memberCnt);
			$.Msg.success('已填写一对多配置');
		},

		addMonthGradeFee() {
			this.monthGradeFeeList.push([null, null, null]);
		},

		delMonthGradeFee(idx) {
			this.monthGradeFeeList.splice(idx, 1);
		},

		genMonthGradeFeeConf() {
			let confs = [];
			let i = 0;
			for (let c of this.monthGradeFeeList) {
				i++;
				if (!c[0] || !c[1] || $.Util.isEmpty(c[2])) {
					$.Dlg.warning('请完整配置包月梯度的第【{0}】行数据'.format(i));
					return null;
				}
				let fromCnt = parseInt(c[0]);
				let toCnt = parseInt(c[1]);
				if (fromCnt > toCnt) {
					$.Dlg.warning('包月梯度的第【{0}】行数据有误'.format(i));
					return null;
				}
				confs.push('{0},{1},{2}'.format(c[0], c[1], c[2]));
			}

			return confs.join(';');
		},

		createMonthGradeFeeList() {
			if (!this.ps.coachLessonFeeConf) return;

			this.monthGradeFeeList = [];
			for (let conf of this.ps.coachLessonFeeConf.split(';')) {
				let items = conf.split(',');
				this.monthGradeFeeList.push([parseInt(items[0]), parseInt(items[1]), parseFloat(items[2])]);
			}
		},
		...
		queryShopArgs() {
			...
			$.Req.service($.SvName.SHOP_ARG_QUERY, args, ret => {
				for (let t of ret.argList) {
					if (t.typeId === 2103) this.disablePsMember = t.typeValue === 'true';
				}
			});
		},
		...
		checkCourseName() {
			if (this.ps.enableMoreStudents) return;

			let courseName = this.getCourseName(this.ps.courseId);
			let checkNames = ['一对二', '一对三', '一对四', '一对五', '一对六', '一对七', '一对八', '一对九'];
			let warning = null;
			for (let cn of checkNames) {
				if (courseName.indexOf(cn) >= 0) {
					warning = '【温馨提示】：该排课未启用多人预约，课程名称【{0}】中不建议包含“{1}”字眼，以免引起约课误解。请检查私教课程是否选择有误，或者启用多人预约，或者修改课程名称（避免使“一对二“，“一对三”，”一对四”等一对多的字眼）。继续提交点击确定，不作提交点击取消。'.format(courseName, cn);
					break;
				}
			}
			return warning;
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__pages_member_member_select_vue__ = __webpack_require__(1123);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__pages_member_member_select_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2__pages_member_member_select_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__components_help__ = __webpack_require__(404);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__components_sys_time_select__ = __webpack_require__(406);
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__pages_course_private_lesson_booking_vue__ = __webpack_require__(1201);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__pages_course_coach_lesson_bgcolor_conf_vue__ = __webpack_require__(1223);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__pages_member_member_base_info_vue__ = __webpack_require__(191);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__pages_member_member_base_info_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2__pages_member_member_base_info_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__pages_member_member_select_vue__ = __webpack_require__(1123);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__components_help_vue__ = __webpack_require__(404);
...
/* harmony default export */ __webpack_exports__["default"] = ({
  components: {
    PrivateLessonBooking: __WEBPACK_IMPORTED_MODULE_0__pages_course_private_lesson_booking_vue___default.a,
    CoachLessonBgcolorConf: __WEBPACK_IMPORTED_MODULE_1__pages_course_coach_lesson_bgcolor_conf_vue___default.a,
    MemberBaseInfo: __WEBPACK_IMPORTED_MODULE_2__pages_member_member_base_info_vue___default.a,
    MemberSelect: __WEBPACK_IMPORTED_MODULE_3__pages_member_member_select_vue___default.a,
  ...
  created() {
    ...
    this.setDateForWeekQuery();
    this.setWeekDates();
    this.queryShopArgs(this.queryPcLessonList);
  ...
  methods: {
    queryShopArgs(callbck = null) {
      let args = {
        typeIdList: [$.Dict.ShopArg.COACH_LESSON_BGCOLOR, 2214, 2215]
      };
      $.Req.service($.SvName.SHOP_ARG_QUERY, args, ret => {
        let coachColorConf = null,
            courseColorConf = null;
    ...
    queryPcLessonList() {
      ...
      if (this.query.isDelayedBooking) {
        args.isDelayedBooking = true;
      }
      $.Req.service($.SvName.PC_LESSON_QUERY, args, ret => {
    ...
    queryBgColorConf(callbck) {
      let args = {
        typeId: this.bgColorType === 1 ? $.Dict.ShopArg.COACH_LESSON_BGCOLOR : 2215
      };
      $.Req.service($.SvName.SHOP_ARG_QUERY, args, ret => {
        this.parseBgColor(ret.value);
        if (callbck) callbck();
    ...
    toLessonPage(ltype, lessonId, mplId) {
      if (ltype === 1) {
        ...
        window.open("#/private-lesson-edit/{0}/{1}".format(lessonId, mplId), "_blank");
      } else {
        $.Msg.info("这是客户体验课");
      }
    },
    ...
    afterSubmitBgColor(bgColorType) {
      this.bgColorType = bgColorType;
      this.viewBgcolorDlg = false;
      this.queryBgColorConf(this.queryPcLessonList);
    },
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__ = __webpack_require__(1122);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__components_page_header_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__components_page_header_vue__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__tenant_tenant_stat_member__ = __webpack_require__(876);
...
// module
exports.push([module.i, ".-style- .tit[data-v-0896d0a4]{margin-bottom:10px}.coach-list li[data-v-0896d0a4]{display:inline-block;margin:5px;text-align:center;color:#fff;line-height:50px;width:105px;border-radius:3px;cursor:pointer}.coach-list li.on[data-v-0896d0a4]{box-shadow:4px 4px 4px #aaa}.coach-list li i[data-v-0896d0a4]{margin-right:2px}.color-list li[data-v-0896d0a4]{width:50px;background:#777}.color-list li[data-v-0896d0a4],.course-list li[data-v-0896d0a4]{display:inline-block;margin:5px;text-align:center;color:#fff;line-height:50px;cursor:pointer}.course-list li[data-v-0896d0a4]{width:180px;border-radius:3px}.course-list li.on[data-v-0896d0a4]{box-shadow:4px 4px 4px #aaa}.course-list li i[data-v-0896d0a4]{margin-right:2px}.color-list .setting-color[data-v-0896d0a4]{text-align:left;background:#fff;color:#333;width:auto;padding:0 10px;border:1px solid #eee}.color-list .setting-color .color[data-v-0896d0a4]{display:flex;align-items:center}.-style- .text-color[data-v-0896d0a4]{margin:15px 0;margin-bottom:15px}.color-input[data-v-0896d0a4]{width:40px;height:40px;cursor:pointer}", ""]);
...
// module
exports.push([module.i, ".-datepick[data-v-1eed0285]{display:inline-block;width:120px}.star[data-v-1eed0285]{color:#2995d8}.query-section .item[data-v-1eed0285]{padding-right:0}.weektimes tr td[data-v-1eed0285],.weektimes tr th[data-v-1eed0285]{padding:10px;font-size:13px}.weektimes tr span[data-v-1eed0285]{margin:5px 10px 5px 0;background:#eee;padding:6px 0;display:inline-block;text-align:center;width:110px;line-height:1}.icon-edit[data-v-1eed0285]{font-size:14px;position:relative;top:2px}.icon-edit[data-v-1eed0285]:hover{color:red}.time-conf .item[data-v-1eed0285]{margin-right:12px}.time-conf .-timepick[data-v-1eed0285]{width:122px;display:inline-block}.exclude-dates[data-v-1eed0285]{display:inline-block}.exclude-dates li[data-v-1eed0285]{display:inline-block;padding-right:6px}.exclude-dates li .input-date[data-v-1eed0285]{width:120px}.-addtime[data-v-1eed0285]{padding:0 9px;position:relative;top:1px}.form-submit[data-v-1eed0285]{margin-top:15px}.lesson-time[data-v-1eed0285]{padding-top:15px;border:1px solid #efefef}", ""]);
...
// module
exports.push([module.i, "section[data-v-9b0d00bc]{margin-bottom:25px}section .tit[data-v-9b0d00bc]{font-size:13px;font-weight:700;padding:5px 0 12px}section .tit[data-v-9b0d00bc]:before{content:\"\\25A0\";padding-right:5px}section .mytbl[data-v-9b0d00bc]{padding:0 15px}section .mytbl th[data-v-9b0d00bc]{font-weight:400}section .mytbl td[data-v-9b0d00bc]{word-break:break-all}", ""]);
...
// module
exports.push([module.i, ".cont .datepick[data-v-ecb9a9fc]{width:120px}.l-status-0[data-v-ecb9a9fc]{color:#aaa}.l-status-9[data-v-ecb9a9fc]{color:#0d9531}.form-submit[data-v-ecb9a9fc]{padding-left:120px}.time-conf .item[data-v-ecb9a9fc]{margin-right:12px}.time-conf .-timepick[data-v-ecb9a9fc]{width:122px;display:inline-block}.-input-s[data-v-ecb9a9fc]{width:120px;display:inline-block}.ps-lesson-title[data-v-ecb9a9fc]{width:180px;display:inline-block}.-addtime[data-v-ecb9a9fc]{padding:0 9px;position:relative;top:1px}.-note[data-v-ecb9a9fc]{color:orange;padding:0 2px 0 8px}.form-submit-bottom[data-v-ecb9a9fc]{padding-left:320px}.members[data-v-ecb9a9fc]{margin:5px 0 10px;padding:10px;border:1px solid #ddd;max-height:300px;overflow-y:auto}.members li[data-v-ecb9a9fc]{display:inline-block;line-height:30px;word-break:keep-all;background:#eee;padding:0 12px;border-radius:2px;margin:4px 8px 4px 0}.members li a.close[data-v-ecb9a9fc]{font-size:16px;padding-left:4px;color:#bbb}.members li a.close[data-v-ecb9a9fc]:hover{color:red}.new-lesson-tbl[data-v-ecb9a9fc]{width:100%;margin-top:15px}.new-lesson-tbl td[data-v-ecb9a9fc],.new-lesson-tbl th[data-v-ecb9a9fc]{padding:6px}.new-lesson-tbl th[data-v-ecb9a9fc]{width:80px;text-align:left}.myform .tit .r[data-v-ecb9a9fc]{font-weight:400}.coach-fee-conf[data-v-ecb9a9fc]{margin-top:10px}.coach-fee-conf li[data-v-ecb9a9fc]{margin:10px 0 0}.coach-fee-conf input[data-v-ecb9a9fc]{width:115px;margin-right:5px}.exclude-dates[data-v-ecb9a9fc]{display:inline-block}.exclude-dates li[data-v-ecb9a9fc]{display:inline-block;padding-right:6px}.exclude-dates li .input-date[data-v-ecb9a9fc]{width:120px}", ""]);
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("9bdef11e", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("18ea27df", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("1cc4c99a", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("04099e96", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("34d22f38", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("6e3cd005", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("55fe5548", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("e84a36de", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("3133fe32", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("8c7db9ca", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("fb4e70be", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("84746afe", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("5806a95a", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("3511a9c4", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("5125c224", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("dc77526a", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("38f03cfa", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("1791bdec", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("4118e1a3", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("111c7110", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("3858692a", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("781cc95d", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("78cdd608", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("5fe98113", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("4a602360", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("fb97e964", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("72f4d1fb", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("1c532434", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("58feb91e", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("60f3c94e", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("71d09d8e", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("65cf9553", content, true, {});
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(874)("67a70332", content, true, {});
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    model: {
      value: (_vm.bgColorType),
      callback: function($$v) {
        _vm.bgColorType = $$v
      },
      expression: "bgColorType"
    }
  }, [_c('radio', {
    attrs: {
      "label": 1
    }
  }, [_vm._v("根据教练设置")]), _vm._v(" "), _c('radio', {
    attrs: {
      "label": 2
    }
  }, [_vm._v("根据课程设置")])], 1)], 1), _vm._v(" "), (_vm.bgColorType === 1) ? _c('ul', {
    staticClass: "coach-list"
  }, _vm._l((_vm.coachList), function(c, idx) {
    return _c('li', {
      key: idx,
      class: {
      ...
      style: ({
        background: '#' + _vm.coachColorMap[c.staffId],
        color: '#' + _vm.coachTextColorMap[c.staffId]
      }),
      on: {
        "click": function($event) {
    ...
    }, [(c.staffId === _vm.selectCoachId) ? _c('i', {
      staticClass: "iconfont icon-check1"
    }) : _vm._e(), _vm._v(_vm._s(c.name) + "\n      ")])
  }), 0) : (_vm.bgColorType === 2) ? _c('ul', {
    staticClass: "course-list"
  }, _vm._l((_vm.courseList), function(c, idx) {
    return _c('li', {
      key: idx,
      class: {
        on: c.courseId === _vm.selectCourseId
      },
      style: ({
        background: '#' + _vm.courseColorMap[c.courseId],
        color: '#' + _vm.courseTextColorMap[c.courseId]
      }),
      on: {
        "click": function($event) {
          return _vm.selectCourse(c.courseId)
        }
      }
    }, [(c.courseId === _vm.selectCourseId) ? _c('i', {
      staticClass: "iconfont icon-check1"
    }) : _vm._e(), _vm._v(_vm._s(c.courseName) + "\n      ")])
  }), 0) : _vm._e()]), _vm._v(" "), _c('div', {
    staticClass: "p15"
  }, [(_vm.selectCoachId || _vm.selectCourseId) ? [_c('div', {
    staticClass: "tit"
  }, [_vm._v("请选择背景色")]), _vm._v(" "), _c('ul', {
    ...
    staticClass: "color"
  }, [_c('span', [_vm._v("更多背景色：")]), _vm._v(" "), _c('input', {
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.bgColor),
    ...
    staticClass: "tit text-color"
  }, [_vm._v("请选择字体颜色")]), _vm._v(" "), _c('ul', {
    staticClass: "color-list"
  }, [_c('input', {
    directives: [{
      name: "model",
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('shop-nav', {
    ...
    model: {
      value: (_vm.query.gcType),
      callback: function($$v) {
        _vm.$set(_vm.query, "gcType", $$v)
      },
      expression: "query.gcType"
    }
  }, _vm._l((_vm.g.Dict.GcTypes), function(g, idx) {
    return _c('i-option', {
      key: idx,
      attrs: {
        "value": idx
  ...
  }, [_vm._v("批量添加上课计费卡")]), _vm._v(" "), _c('i-button', {
    on: {
      "click": _vm.openUpateGcTypeDlg
    }
  }, [_vm._v("批量设置课程类型")])], 1), _vm._v(" "), _c('div', {
    staticClass: "r"
  }, [_c('checkbox', {
    on: {
      "on-change": function($event) {
  ...
  }, [_vm._v("课程难度")]), _vm._v(" "), _c('th', {
    attrs: {
      "width": "70"
    }
  }, [_vm._v("课程类型")]), _vm._v(" "), _c('th', [_vm._v("课程/排课名称")]), _vm._v(" "), _c('th', {
    attrs: {
      "width": "70"
    }
    ...
    return _c('tr', {
      ...
      staticClass: "iconfont icon-checkbox-checked ckbox-1"
    })]) : _vm._e(), _vm._v(" "), _c('td', [(s.lessonLevel) ? [_vm._v(_vm._s(s.lessonLevel) + "级")] : _vm._e()], 2), _vm._v(" "), _c('td', [(_vm.g.Util.isNotEmpty(s.gcType)) ? [_vm._v(_vm._s(_vm.g.Dict.GcTypes[s.gcType]))] : _vm._e()], 2), _vm._v(" "), _c('td', [_c('a', {
      staticClass: "block",
      attrs: {
        "href": '#/group-course-schedule-edit/' + s.csId
      }
    }, [_vm._v("\n\t\t\t\t\t\t\t\t\t" + _vm._s(s.courseName) + "\n\t\t\t\t\t\t\t\t\t"), (s.csName) ? [_vm._v("/ " + _vm._s(s.csName))] : _vm._e()], 2)]), _vm._v(" "), _c('td', [_vm._v(_vm._s(s.coachName))]), _vm._v(" "), (_vm.g.data.user.isShopOwner) ? _c('td', [(s.coachLessonFee && s.coachLessonFeeType < 2) ? _c('span', [_vm._v("￥" + _vm._s(s.coachLessonFee))]) : _vm._e()]) : _vm._e(), _vm._v(" "), _c('td', [_c('span', {
      staticClass: "pointer",
      on: {
        "click": function($event) {
    ...
    }, [_vm._v(_vm._s(_vm.g.Util.formatDate(s.beginDate)) + " ~ " + _vm._s(_vm.g.Util.formatDate(s.endDate)))]), _vm._v(" "), _c('a', {
      staticClass: "l5",
      attrs: {
        "title": "课程周期"
      },
    ...
    })]), _vm._v(" "), (_vm.courseEditable) ? _c('a', {
      staticClass: "l5",
      attrs: {
        "title": "修改日期"
      },
      ...
      staticClass: "iconfont icon-edit gray"
    })]) : _vm._e()]), _vm._v(" "), _c('td', [_vm._v(_vm._s(s.groundName))]), _vm._v(" "), _c('td', [(s.isNeedBooking) ? [_vm._v("是")] : _c('span', {
      staticClass: "gray"
    ...
    model: {
      value: (_vm.isBatchAppendCards),
      callback: function($$v) {
        _vm.isBatchAppendCards = $$v
      },
      expression: "isBatchAppendCards"
    }
  }, [_vm._v("批量选择会员卡")])], 1)]), _vm._v(" "), (_vm.isBatchAppendCards) ? _c('tr', [_c('th', [_vm._v("卡类型")]), _vm._v(" "), _c('td', [_c('i-select', {
    staticStyle: {
      "width": "112px"
    },
    attrs: {
    ...
    model: {
      value: (_vm.selectCardType),
      callback: function($$v) {
        _vm.selectCardType = $$v
      },
      expression: "selectCardType"
    }
  ...
  }), 1)], 1)]) : _vm._e(), _vm._v(" "), _c('tr', [_c('th', [_vm._v("会员卡")]), _vm._v(" "), _c('td', [(_vm.isBatchAppendCards) ? [(_vm.cardList) ? _c('i-select', {
    attrs: {
      "filterable": "",
      "multiple": ""
    },
    ...
    return (c.cardType === _vm.selectCardType) ? _c('i-option', {
      key: c.cardId,
      attrs: {
        "value": c.cardId,
        "label": c.cardName
  ...
  }), 1) : _vm._e()] : [(_vm.cardList) ? _c('i-select', {
    attrs: {
      "filterable": ""
    },
    on: {
  ...
  }), 1) : _vm._e()]], 2)]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("上课计费")]), _vm._v(" "), _c('td', [(_vm.isBatchAppendCards) ? [(_vm.selectCardType > 1) ? [_c('input-num', {
    staticStyle: {
      "width": "100px"
    },
    attrs: {
  ...
  }), _vm._v("\n\t\t\t\t\t\t\t\t\t  " + _vm._s(_vm.g.Dict.CardUnitName[_vm.selectCardType]) + "\n\t\t\t\t\t\t\t\t")] : _c('span', {
    staticClass: "orangered"
  }, [_vm._v("( 期限卡不计费 )")])] : [(_vm.selectCard && _vm.selectCard.cardType === 1) ? _c('span', {
    staticClass: "orangered"
  }, [_vm._v("( 期限卡不计费 )")]) : _vm._e(), _vm._v(" "), (!_vm.selectCard || (_vm.selectCard && _vm.selectCard.cardType > 1)) ? _c('input-num', {
    staticStyle: {
      "width": "100px"
    },
    attrs: {
  ...
  }) : _vm._e(), _vm._v(" "), (_vm.selectCard && _vm.selectCard.cardType > 1) ? [_vm._v("   " + _vm._s(_vm.g.Dict.CardUnitName[_vm.selectCard.cardType]))] : _vm._e()]], 2)]), _vm._v(" "), _c('tr', [_c('th'), _vm._v(" "), _c('td', [_c('div', {
    staticClass: "p10"
  }, [_c('i-button', {
    staticStyle: {
      "width": "100px"
  ...
  }, [(_vm.csWeektimesConfDlg && _vm.selectCs) ? _c('div', {
    staticClass: "mytbl weektimes"
  }, [_c('table', [_c('tr', [_c('th', {
    attrs: {
      "width": "100"
    }
  }, [_vm._v("课程日期")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Util.formatDate(_vm.selectCs.beginDate)) + " ~ " + _vm._s(_vm.g.Util.formatDate(_vm.selectCs.endDate)))])]), _vm._v(" "), (_vm.selectCs.excludeDates) ? _c('tr', [_c('th', [_vm._v("排除日期")]), _vm._v(" "), _c('td', _vm._l((_vm.selectCs.excludeDates.split(',')), function(d) {
    return _c('span', [_vm._v(_vm._s(d))])
  }), 0)]) : _vm._e(), _vm._v(" "), _vm._l(([_vm.selectCs.week1Times, _vm.selectCs.week2Times, _vm.selectCs.week3Times, _vm.selectCs.week4Times, _vm.selectCs.week5Times, _vm.selectCs.week6Times, _vm.selectCs.week7Times]), function(w, idx) {
    return _c('tr', [_c('th', [_vm._v(_vm._s(_vm.g.Dict.WeekName[(idx + 1) % 7]))]), _vm._v(" "), _c('td', [(w) ? _vm._l((w.split(',')), function(item) {
    ...
    model: {
      value: (_vm.updateCsGcTypesDlg),
      callback: function($$v) {
        _vm.updateCsGcTypesDlg = $$v
      },
      expression: "updateCsGcTypesDlg"
    }
  }, [_c('div', {
    staticClass: "mb12"
  }, [_vm._v("请选择更新后的课程类型：")]), _vm._v(" "), _c('radio-group', {
    model: {
      value: (_vm.selectGcType),
      callback: function($$v) {
        _vm.selectGcType = $$v
      },
      expression: "selectGcType"
    }
  }, [_c('radio', {
    attrs: {
      "label": -1
    }
  }, [_c('i', [_vm._v("无")])]), _vm._v(" "), _vm._l((_vm.g.Dict.GcTypes), function(name, val) {
    return _c('radio', {
      key: val,
      attrs: {
        "label": val
    ...
    on: {
      "click": _vm.updateCsGcType
    }
  }, [_vm._v("提交")]), _vm._v(" "), _c('i-button', {
    on: {
      "click": function($event) {
        _vm.updateCsGcTypesDlg = false
      }
    }
    ...
    staticClass: "warning"
  }, [_vm._v("\n\t\t\t\t备注：修改课程日期后，系统将会自动重新排课（已预约或签到的课不受影响）\n\t\t\t")]), _vm._v(" "), (!_vm.quickEditTime) ? [_c('div', {
    staticClass: "mytbl weektimes mt20 mb12"
  }, [(_vm.selectCs) ? _c('table', [_c('tr', [_c('th', {
    attrs: {
      "width": "100"
    }
  }, [_vm._v("课程日期")]), _vm._v(" "), _c('td', [_c('date-picker', {
    staticClass: "-input-s",
    attrs: {
      "type": "date",
      "placeholder": "开始日期"
  ...
  })], 1)]), _vm._v(" "), (_vm.selectCs.excludeDates) ? _c('tr', [_c('th', [_vm._v("排除日期")]), _vm._v(" "), _c('td', _vm._l((_vm.selectCs.excludeDates.split(',')), function(d) {
    return _c('span', [_vm._v(_vm._s(d))])
  }), 0)]) : _vm._e(), _vm._v(" "), _vm._l(([_vm.selectCs.week1Times, _vm.selectCs.week2Times, _vm.selectCs.week3Times, _vm.selectCs.week4Times, _vm.selectCs.week5Times, _vm.selectCs.week6Times, _vm.selectCs.week7Times]), function(w, idx) {
    return _c('tr', [_c('th', [_vm._v(_vm._s(_vm.g.Dict.WeekName[(idx + 1) % 7]))]), _vm._v(" "), _c('td', [(w) ? _vm._l((w.split(',')), function(item) {
      return _c('span', [_vm._v("\n\t\t\t\t\t\t\t\t\t" + _vm._s(_vm.g.Util.formatTime(item.split('-')[0])) + " ~ " + _vm._s(_vm.g.Util.formatTime(item.split('-')[1])) + "\n\t\t\t\t\t\t\t\t")])
    ...
    staticClass: "mt20 mb12 lesson-time"
  }, [_c('i-form', {
    ...
    model: {
      value: (_vm.cs.publicTime),
      callback: function($$v) {
        _vm.$set(_vm.cs, "publicTime", $$v)
      },
      expression: "cs.publicTime"
    }
  }), _vm._v(" "), _c('span', {
    staticClass: "orange l15"
  }, [_vm._v("（ 如果不填写，则表示立即发布生成的课；否则，按照填写的时间发布到手机端 ）")])], 1)], 2)], 1)], _vm._v(" "), _c('div', {
    staticClass: "p5"
  }, [_c('checkbox', {
    on: {
      "on-change": function($event) {
    ...
    model: {
      value: (_vm.quickEditTime),
      callback: function($$v) {
        _vm.quickEditTime = $$v
      },
      expression: "quickEditTime"
    }
  }, [_vm._v("编辑上课时间")])], 1), _vm._v(" "), _c('div', {
    staticClass: "form-submit center"
  }, [_c('i-button', {
    attrs: {
      "type": "primary"
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v("团课取消日志")]), _vm._v(" "), _c('span', {
    staticClass: "spt"
  }, [_vm._v(" | ")]), _vm._v(" "), _c('a', {
    on: {
      "click": function($event) {
        return _vm.$router.push('/cust-group-lesson')
      }
    }
  }, [_vm._v("团课体验课")])])])])]), _vm._v(" "), _c('section', [_c('div', {
    staticClass: "tbl-header"
  }, [_c('table', {
    attrs: {
      "width": "100%"
  ...
  }), _vm._v("添加一节课")])])], 1) : (_vm.lessonList) ? _c('span', [(!_vm.lessonCount) ? [_vm._v("共 " + _vm._s(_vm.lessonList.length) + " 节课")] : [_vm._v("共 " + _vm._s(_vm.lessonCount) + " 节课")], _vm._v(" "), (_vm.viewType === 2) ? _c('span', {
    staticClass: "l15 more-opt"
  }, [_c('i-button', {
    staticClass: "btn"
  }, [_vm._v("更多 "), _c('Icon', {
    attrs: {
  ...
  })], 1), _vm._v(" "), _c('ul', [_c('li', [_c('checkbox', {
    on: {
      "on-change": function($event) {
        return _vm.queryLessonList(null)
      }
  ...
  }, [_vm._v("显示停用课程的课")])], 1), _vm._v(" "), _c('li', [_c('checkbox', {
    model: {
      value: (_vm.isCombineLessonDate),
      callback: function($$v) {
        _vm.isCombineLessonDate = $$v
      },
      expression: "isCombineLessonDate"
    }
  }, [_vm._v("合并同一日期")])], 1), _vm._v(" "), _c('li', {
    on: {
      "click": function($event) {
        return _vm.queryLessonList(1, true)
      }
    }
  }, [_c('Icon', {
    attrs: {
  ...
  }), _vm._v("导出课表")], 1)])], 1) : _vm._e()], 2) : _vm._e()])])])]), _vm._v(" "), (_vm.query.dateType === 'week') ? _c('div', {
    staticClass: "mytbl",
    attrs: {
    ...
    return _c('th', {
      ...
      style: ({
        ...
        color: _vm.weekDates[idx].compareDatePart(_vm.today) === 0 ? '#FF4500FF' : ''
      })
    ...
    return _c('tr', [_c('td', {
      ...
      return _c('td', {
        ...
        on: {
          "click": function($event) {
            return _vm.clickLessonTable(bt, w)
          }
        }
      }, [(_vm.lessonMap[bt][w]) ? _c('table', _vm._l((_vm.lessonMap[bt][w]), function(l) {
        return _c('tr', {
          on: {
            "click": function($event) {
          ...
          style: ({
            background: '#' + _vm.bgColorMap[_vm.bgColorType === 1 ? l.coachId : l.courseId],
            color: '#' + _vm.textColorMap[_vm.bgColorType === 1 ? l.coachId : l.courseId]
          })
    ...
    return _c('tr', _vm._l(([1, 2, 3, 4, 5, 6, 7]), function(j) {
      return _c('td', {
      ...
      }, [(rows[j]) ? [_c('table', [_c('tr', [_c('td', {
        class: {
          'day': true,
          'on': _vm.todayStr === rows[j].date.format('yyyyMMdd')
        },
        style: ({
          color: (_vm.todayStr === rows[j].date.format('yyyyMMdd') ? 'red' : ''),
          background: '#ccc',
          textAlign: 'left'
        })
      }, [_vm._v("\n\t\t\t\t\t\t\t\t\t\t\t" + _vm._s(rows[j].date.getDate()) + "\n\t\t\t\t\t\t\t\t\t\t\t"), (!_vm.g.Util.isEmptyArray(rows[j].lessons)) ? _c('span', {
        staticClass: "l5 gray"
      }, [_vm._v("(" + _vm._s(rows[j].lessons.length) + "节)")]) : _vm._e()])])]), _vm._v(" "), (rows[j].lessons && rows[j].lessons.length > 0) ? _c('table', _vm._l((rows[j].lessons), function(l) {
        return _c('tr', {
          staticStyle: {
            "padding": "10px"
          },
          ...
          style: ({
            color: '#' + (l.status == 0 ? '444' : (l.status == 1 ? '1AAD19' : '999'))
          })
        }, [_c('span', {
          staticStyle: {
            "font-size": "15px",
        ...
        }, [_vm._v("·")]), _vm._v(_vm._s(l.coachName) + "@" + _vm._s(l.beginTimeDesc) + " " + _vm._s(l.lessonTitle) + "\n\t\t\t\t\t\t\t\t\t\t\t"), (_vm.showBookings) ? _c('div', {
          staticClass: "gray"
    ...
    staticClass: "mytbl",
    attrs: {
      "id": "groupLesonListTbl"
    }
  }, [_c('table', [_vm._m(0), _vm._v(" "), _vm._l((_vm.lessonList), function(l, idx) {
    return _c('tr', [(_vm.isCombineLessonDate) ? [(idx === 0 || (l.lessonDate !== _vm.lessonList[idx - 1].lessonDate)) ? _c('td', {
      staticClass: "center",
      attrs: {
    ...
    }, [_vm._v("（" + _vm._s(_vm.lessonDateCntMap[l.lessonDate]) + " 节）")]) : _vm._e()]) : _vm._e()] : _c('td', {
      staticClass: "center"
    }, [_c('span', {
      class: {
        'orangered': _vm.today.compareDatePart(l.lessonDate) === 0
      }
    }, [_vm._v(_vm._s(_vm.g.Util.formatDate(l.lessonDate)))])]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Util.formatTime(l.beginTime)) + " ~ " + _vm._s(_vm.g.Util.formatTime(l.endTime)) + " ")]), _vm._v(" "), _c('td', [_c('a', {
      staticClass: "block",
      on: {
        "click": function($event) {
          return _vm.viewGroupLessonRecords(l)
    ...
    }, [_vm._v(_vm._s(l.lessonTitle))])]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.coachName.shorten(4)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.groundName))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.checkinTime))]), _vm._v(" "), _c('td', [(l.isNeedBooking) ? [(l.bookings) ? [_vm._v(_vm._s(l.bookings))] : _c('span', {
      staticClass: "gray"
    }, [_vm._v("0")])] : _vm._e()], 2), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.classMemberCnt))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.checkins))]), _vm._v(" "), _c('td', [(l.checkins) ? [(l.isNeedBooking && l.bookings && l.checkins && l.bookings >= l.checkins) ? _c('span', [_vm._v("\n\t\t\t\t\t\t\t\t\t" + _vm._s(_vm.g.Util.round(100 * l.checkins / l.bookings)) + "%\n\t\t\t\t\t\t\t\t")]) : _vm._e(), _vm._v(" "), (!l.isNeedBooking && l.classMemberCnt && l.classMemberCnt >= l.checkins) ? _c('span', [_vm._v("\n\t\t\t\t\t\t\t\t\t" + _vm._s(_vm.g.Util.round(100 * l.checkins / l.classMemberCnt)) + "%\n\t\t\t\t\t\t\t\t")]) : _vm._e()] : _vm._e()], 2), _vm._v(" "), _c('td', [(l.maxStudents) ? [(l.bookings) ? [_vm._v(_vm._s(_vm.g.Util.round(100 * l.bookings / l.maxStudents)) + "%")] : _c('span', {
      staticClass: "gray"
    }, [_vm._v("0")])] : _vm._e()], 2), _vm._v(" "), _c('td', [_c('ul', {
      staticClass: "opt-list"
    ...
    }, [_vm._v("评价(" + _vm._s(l.memberReviewCnt) + ")")])]) : _vm._e()])])], 2)
  }), _vm._v(" "), _c('tr', [_c('td', {
    attrs: {
      "colspan": "6",
      "align": "center"
    ...
    staticClass: "bold"
  }, [_vm._v(_vm._s(_vm.sum.checkins))]), _vm._v(" "), _c('td'), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.sum.avgBookingRates) + "%")]), _vm._v(" "), _c('td')])], 2), _vm._v(" "), (!_vm.lessonCount && !_vm.lessonList) ? _c('div', {
  ...
  }, [_vm._v("删除本节课")])] : _vm._e()], 2) : _vm._e()], 2), _vm._v(" "), _c('modal', {
    attrs: {
      "title": "导出文件下载",
      "width": "500",
      "ok-text": "关闭",
  ...
  }), _vm._v(" 快速生成一节课； 如需生成若干节课，请使用 "), _c('a', {
    on: {
      "click": function($event) {
        return _vm.$router.push('/group-course-schedule')
      }
    }
  }, [_vm._v("【团课排课】")]), _vm._v(" 功能。")]), _vm._v(" "), (_vm.newLessonDlg) ? _c('table', {
    ...
    staticClass: "required"
  }, [_vm._v("排课")]), _vm._v(" "), _c('td', [_c('i-select', {
    staticStyle: {
      "width": "150px"
    },
    attrs: {
    ...
    model: {
      value: (_vm.newLesson.csId),
      callback: function($$v) {
        _vm.$set(_vm.newLesson, "csId", $$v)
      },
      expression: "newLesson.csId"
    }
  }, _vm._l((_vm.courseScheduleList), function(c) {
    return _c('i-option', {
      key: c.csId,
      attrs: {
        "value": c.csId
  ...
  }), 1)], 1)]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("上课主题")]), _vm._v(" "), _c('td', [_c('i-input', {
    staticStyle: {
      "width": "150px"
    },
    attrs: {
    ...
    model: {
      value: (_vm.newLesson.lessonTitle),
      callback: function($$v) {
        _vm.$set(_vm.newLesson, "lessonTitle", $$v)
      },
      expression: "newLesson.lessonTitle"
    }
  })], 1)]), _vm._v(" "), _c('tr', [_c('th', [_vm._v("上课助教")]), _vm._v(" "), _c('td', [_c('i-select', {
    attrs: {
      "multiple": ""
    },
    model: {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: "-date-tit"
  }, [_vm._v(_vm._s(_vm.query.dateDesc)), (_vm.query.isDelayedBooking) ? _c('span', {
    staticClass: "red big"
  }, [_vm._v("（补约）")]) : _vm._e()]), _vm._v(" "), (_vm.query.dateType === 'week') ? _c('span', {
    staticClass: "-qdate-nav"
  }, [_c('a', {
    on: {
      "click": function($event) {
  ...
  }, [_vm._v("设置背景色")])], 1) : _vm._e(), _vm._v(" "), _c('li', {
    staticClass: "item"
  }, [_c('Dropdown', {
    attrs: {
      "placement": "bottom-end",
    ...
    on: {
      "on-change": _vm.filterDelayedBooking
    },
    model: {
      value: (_vm.query.isDelayedBooking),
      callback: function($$v) {
        _vm.$set(_vm.query, "isDelayedBooking", $$v)
      },
      expression: "query.isDelayedBooking"
    }
  }, [_vm._v("补约记录")])], 1), _vm._v(" "), _c('DropdownItem', {
    attrs: {
      "name": "print"
    }
    ...
    return _c('tr', {
      ...
      staticClass: "custflag"
    }, [_vm._v("客")]), _vm._v(_vm._s(l.rawMemberName))]], 2), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.createUname))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.createTime))]), _vm._v(" "), _c('td', {
      class: {
        orange: l.checkinUname && (l.rawMemberName !== l.checkinUname)
      }
    }, [_vm._v(_vm._s(l.checkinUname))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.checkinTime))]), _vm._v(" "), _c('td', [_c('ul', {
      staticClass: "opts"
    ...
    return _c('tr', [_c('td', {
      ...
      return _c('td', {
        ...
        return _c('li', {
          ...
          attrs: {
            "title": '预约人/预约时间：' + l.createUname + ' / ' + l.createTime
          },
          on: {
            "click": function($event) {
              return _vm.toLessonPage(l.ltype, l.lessonId, l.mplId)
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    slot: "content"
  }, [_vm._v("\n\t\t\t\t\t\t\t对于中小规模的或需求简要的场馆，"), _c('br'), _vm._v("只需要选择和填写极简模式中的数据；\n\t\t\t\t\t\t\t"), _c('p'), _vm._v("\n\t\t\t\t\t\t\t对于规模较大或要求较高的场馆，"), _c('br'), _vm._v("可以选择专业模式，填写更多排课数据。\n\t\t\t\t\t\t")])]), _vm._v(" "), _c('radio-group', {
    on: {
      "on-change": _vm.changeDisplayView
    },
    model: {
  ...
  }, [_vm._v("精简模式")]), _vm._v(" "), _c('radio', {
    attrs: {
      "label": 2
    }
  }, [_vm._v("专业模式")])], 1), _vm._v(" "), (_vm.ps && _vm.ps.psId && (_vm.courseEditable || _vm.ps.coachId === _vm.g.data.user.staffId)) ? _c('span', [_c('span', {
    staticClass: "spt"
  }, [_vm._v("|")]), _vm._v("  \n\t\t\t\t\t\t"), (_vm.tab === 'detail') ? _c('a', {
    on: {
      "click": function($event) {
  ...
  }, [_vm._v("返回")])] : _vm._e()], 2) : _vm._e()], 1)]), _vm._v(" "), (_vm.tab === 'detail') ? _c('div', {
    staticClass: "myform",
    staticStyle: {
      "width": "100%"
    }
    ...
    staticClass: "tit"
  }, [_vm._v("\n\t\t\t\t\t基本信息\n\t\t\t\t\t"), (!_vm.ps.memberId) ? _c('div', {
    staticClass: "r ps-help"
  }, [_c('Poptip', {
    attrs: {
      "placement": "bottom-end",
      "width": "380",
    ...
    staticClass: "iconfont icon-svginfor red"
  }), _vm._v(" 如何设置一对多私教课?")]), _vm._v(" "), _c('div', {
    staticClass: "cont",
    attrs: {
      "slot": "content"
    },
    slot: "content"
  }, [_vm._v("\n\t\t\t\t\t\t\t\t一对多私教课是指一节私教课可同时有多个学员上课，操作如下：\n\t\t\t\t\t\t\t\t"), _c('ol', {
    staticClass: "p15"
  }, [_c('li', [_vm._v("选择视图模式为“专业模式”")]), _vm._v(" "), _c('li', [_vm._v("启用多人预约")]), _vm._v(" "), _c('li', [_vm._v("填写“最多预约人数”")])]), _vm._v(" "), _c('div', {
    staticClass: "p15"
  }, [_c('b', [_vm._v("快捷设置：")]), _vm._v(" "), _c('a', {
    on: {
      "click": function($event) {
        return _vm.setMoreLessonMembers(2)
      }
    }
  }, [_vm._v("点击设置一对二私教课")]), _vm._v(" "), _c('span', {
    staticClass: "spt"
  }, [_vm._v("|")]), _vm._v(" "), _c('a', {
    on: {
      "click": function($event) {
        return _vm.setMoreLessonMembers(3)
      }
    }
  }, [_vm._v("点击设置一对三私教课")])])])])], 1) : _vm._e()]), _vm._v(" "), _c('i-form', {
    ref: "ps",
    staticStyle: {
      "width": "750px"
    },
  ...
  }, [_vm._v(_vm._s(_vm.ps.memberName))])]) : (!_vm.disablePsMember) ? _c('form-item', {
    attrs: {
      "prop": "memberId",
      "label": "会员"
    }
  ...
  }, [(!_vm.member.name) ? _c('span', {
    staticClass: "gray"
  }, [_vm._v("可选填")]) : _c('span', [_vm._v(_vm._s(_vm.member.name))]), _vm._v("  \n\t\t\t\t\t\t\t")]), _vm._v(" "), _c('div', {
    staticClass: "r",
    attrs: {
      "title": "清除会员"
    },
  ...
  }, [_vm._v("【查看会员信息】")])]) : _vm._e(), _vm._v(" "), _c('span', {
    staticClass: "orangered l10"
  }, [_c('i', {
    staticClass: "iconfont icon-alert"
  }), _vm._v("\n\t\t\t\t\t\t\t如不设置表示有如下计费卡的会员都可预约，否则仅此会员可约\n            ")]), _vm._v(" "), (_vm.isMultiCoachs && _vm.ps.coachIds && _vm.member.name) ? _c('div', {
    staticClass: "warning mt15 big bold",
    staticStyle: {
      "line-height": "1.5"
    }
  }, [_c('i', {
    staticClass: "iconfont icon-alert biggest"
  }), _vm._v("\n\t\t\t\t\t\t\t给教练批量排课时，强烈建议不用设置具体会员，否则每进一个新会员都要排课，显得非常繁琐和不明智。\n\t\t\t\t\t\t")]) : _vm._e()]) : _vm._e(), _vm._v(" "), _c('form-item', {
    attrs: {
      "prop": "duration",
      "label": "单次课时长"
    }
    ...
    staticClass: "tips"
  }, [_vm._v("分钟")])], 1), _vm._v(" "), ((_vm.member.memberId || !_vm.g.Util.isEmptyArray(_vm.limitMembers)) && _vm.displayView === 2) ? _c('form-item', {
    attrs: {
      "prop": "limitTimes",
      "label": "排课次数"
    }
    ...
    staticClass: "tips"
  }, [_vm._v("次")])]) : _vm._e(), _vm._v(" "), (_vm.displayView === 2) ? _c('form-item', {
    attrs: {
      "label": "课程有效期"
    }
  }, [_c('date-picker', {
    ...
    staticClass: "tips"
  }, [_vm._v("(可选填；如果填写，则约课时仅限制在此范围内上课，否则不限制)")])], 1) : _vm._e(), _vm._v(" "), (_vm.displayView === 2) ? _c('form-item', {
    attrs: {
      "prop": "groundId",
      "label": "上课场地"
    }
  ...
  }), 1)], 1) : _vm._e(), _vm._v(" "), (!_vm.member.memberId && _vm.displayView === 2) ? [_c('form-item', {
    attrs: {
    ...
    model: {
      value: (_vm.ps.lessonEndType),
      callback: function($$v) {
        _vm.$set(_vm.ps, "lessonEndType", $$v)
      },
      expression: "ps.lessonEndType"
    }
  }, [_c('Radio', {
    attrs: {
      "label": 0
    }
  }, [_vm._v("最少签到 "), _c('input-int', {
    staticClass: "center",
    staticStyle: {
      "width": "75px"
    },
    ...
    model: {
      ...
      callback: function($$v) {
        _vm.$set(_vm.ps, "minCheckinsForEnd", $$v)
      },
      expression: "ps.minCheckinsForEnd"
    }
  }), _vm._v(" 人\n\t\t\t\t\t\t\t\t\t"), _c('help', [_c('i', {
    staticClass: "iconfont icon-alert"
  }), _vm._v(" 表示最少签到多少人，本节课才结束；默认为1人")])], 1), _vm._v(" "), _c('Radio', {
    attrs: {
      "label": 1
    }
  }, [_vm._v("预约会员全部签到")])], 1)], 1) : _vm._e(), _vm._v(" "), (_vm.ps.enableMoreStudents) ? _c('form-item', {
    attrs: {
  ...
  }, [_vm._v("根据签到人数自定义")]), _vm._v(" "), _c('Radio', {
    attrs: {
      "label": 4,
      "disabled": !_vm.psCoachLessonFeeEditable
    }
  }, [_vm._v("包月梯度"), _c('help', [_vm._v("按照每个会员每个月上课次数梯度设置不同的课时费")])], 1)], 1), _vm._v(" "), (_vm.ps.coachLessonFeeType === 2 && _vm.psCoachLessonFeeEditable) ? _c('div', {
    staticClass: "p10"
  }, [_c('i-button', {
    on: {
      "click": function($event) {
  ...
  }, [_vm._v("\n\t\t\t\t\t\t\t\t\t会员卡单次消费\n\t\t\t\t\t\t\t\t\t"), _c('help', [_vm._v("对于期限卡，等于平均每天的价格；"), _c('br'), _vm._v("对于次数卡，等于平均每次的价格；"), _c('br'), _vm._v("对于储值卡，等于本节课的实际消费金额")])], 1), _vm._v(" "), _c('Radio', {
    attrs: {
      "label": 0,
      "disabled": !_vm.psCoachLessonFeeEditable
    }
  ...
  }, [_vm._v("包月梯度"), _c('help', [_vm._v("按照每个会员每个月上课次数梯度设置不同的课时费")])], 1)], 1)], 1) : _vm._e(), _vm._v(" "), (_vm.ps.coachLessonFeeType === 0 || _vm.ps.coachLessonFeeType === 3) ? _c('form-item', {
    attrs: {
      "prop": "coachLessonFee",
    ...
    staticClass: "iconfont icon-alert icon"
  }), _vm._v(" 此数据编辑需要有【私教排课课时费编辑】权限")]) : _vm._e()], 1) : _vm._e(), _vm._v(" "), (_vm.ps.coachLessonFeeType === 4) ? _c('form-item', {
    staticClass: "month-grade"
  }, [_c('ol', _vm._l((_vm.monthGradeFeeList), function(c, idx) {
    return _c('li', [_vm._v("\n\t\t\t\t\t\t\t\t\t第"), _c('input-int', {
      model: {
        value: (c[0]),
        callback: function($$v) {
      ...
      on: {
        "click": function($event) {
          return _vm.delMonthGradeFee(idx)
        }
      }
    }, [_vm._v("删除")])])], 1)
  }), 0), _vm._v(" "), _c('div', {
    ...
    on: {
      "click": _vm.addMonthGradeFee
    }
  }, [_vm._v("添加")])], 1)]) : _vm._e()], _vm._v(" "), _c('form-item', {
    attrs: {
      "label": "上课计费卡设置"
    }
  }, [_c('span', {
  ...
  }, [_vm._v("显示停用卡种")])], 1)], 1) : _vm._e(), _vm._v(" "), (_vm.memberCardIdMap) ? _c('ul', {
    staticClass: "card-payment-list"
  }, [_c('li', {
    staticClass: "-hd"
  }, [_c('div', {
    staticClass: "name"
  }, [(_vm.ps.memberId || _vm.member.name) ? [_vm._v("会员卡种类")] : _c('checkbox', {
    on: {
  ...
  }, [_vm._v("\n\t\t\t\t\t\t\t\t\t\t会员卡种类 " + _vm._s(_vm.member.name) + "\n\t\t\t\t\t\t\t\t\t")])], 2), _vm._v(" "), _c('div', {
    staticClass: "desc"
  }, [_vm._v("课程结算设置")])]), _vm._v(" "), _vm._l((_vm.cardList), function(c, index) {
    return (((!_vm.member.memberId) || (_vm.member.memberId && _vm.memberCardIdMap[c.cardId])) && _vm.cardClassList.contains(c.cardClass) && (c.status === 0 || _vm.showPausedCards)) ? _c('li', [_c('div', {
      staticClass: "name"
    }, [_c('checkbox', {
      on: {
        "on-change": function($event) {
    ...
    return (_vm.cardPayments[mc.cardId] && _vm.cardPayments[mc.cardId].checked) ? _c('Checkbox', {
      ...
      staticClass: "gray"
    }, [_vm._v("/ " + _vm._s(_vm.g.Util.formatDate(mc.endDate)) + "到期")]) : (mc.cardType === 2) ? _c('span', {
      staticClass: "gray"
    ...
    staticClass: "l5"
  }, [(_vm.ps.extraCardId) ? [_vm._v(_vm._s(_vm.g.Dict.CardUnitName[_vm.cardMap[_vm.ps.extraCardId].cardType]))] : _vm._e()], 2), _vm._v(" "), _c('span', {
    staticClass: "l15"
  }, [_c('i-button', {
    attrs: {
      "icon": "md-add"
    ...
    on: {
      "click": _vm.addMoreExtrCardConf
    }
  }, [_vm._v("追加")])], 1), _vm._v(" "), _c('help', {
    staticClass: "l10"
  }, [_vm._v("\n\t\t\t\t\t\t\t\t额外计费项，是指除了常规预约计费外，场馆还需要额外扣除会员的一笔会员卡费用（即此处编辑的会员卡与费用）\n\t\t\t\t\t\t\t\t"), _c('p'), _vm._v("\n\t\t\t\t\t\t\t\t此处的配置不影响会员正常预约，仅当预约上课的会员存在此处配置的会员卡时，才额外计费。\n\t\t\t\t\t\t\t\t"), _c('p'), _vm._v("\n\t\t\t\t\t\t\t\t当配置多个计费卡时，系统只按顺序选择会员的一个有效卡计费。\n\t\t\t\t\t\t\t")]), _vm._v(" "), _vm._l((_vm.moreExtrCardConfList), function(c) {
    return _c('div', {
      staticClass: "p10"
    }, [_c('i-select', {
      staticClass: "extr-card-sel",
      ...
      model: {
        value: (c.cardPayment),
        callback: function($$v) {
          _vm.$set(c, "cardPayment", $$v)
        },
        expression: "c.cardPayment"
      }
    }), _vm._v(" "), _c('span', {
      staticClass: "l5"
    }, [(c.cardId) ? [_vm._v(_vm._s(_vm.g.Dict.CardUnitName[_vm.cardMap[c.cardId].cardType]))] : _vm._e()], 2)], 1)
  })], 2), _vm._v(" "), _c('form-item', {
    attrs: {
      "prop": "ignoreExtraCardCheck",
      "label": "额外计费卡检查"
    }
  }, [_c('i-switch', {
    attrs: {
    ...
    model: {
      value: (_vm.needExtraCardCheck),
      callback: function($$v) {
        _vm.needExtraCardCheck = $$v
      },
      expression: "needExtraCardCheck"
    }
  }, [_c('span', {
    attrs: {
      "slot": "open"
    ...
    staticClass: "tips"
  }, [_vm._v("（选择“否”后，预约时将不检查额外计费卡，即余额不足时也不影响预约）")])], 1)] : _vm._e(), _vm._v(" "), (_vm.displayView === 2) ? _c('form-item', {
    attrs: {
      "prop": "limitWeeks",
      "label": "限定上课星期"
    }
  ...
  }, [_vm._v("周日")])], 1), _vm._v(" "), _c('help', [_vm._v("\n\t\t\t\t\t\t\t全不选（默认）表示不限制，相当于全选。如果勾选部分星期，表示只限定规定的星期可上课。\n\t\t\t\t\t\t\t"), _c('p', [_vm._v("例如，勾选了“周六、周日”，表示只能预约每周的周六和周日上课。")])])], 1) : _vm._e(), _vm._v(" "), (!_vm.member.memberId && _vm.displayView === 2) ? _c('form-item', {
    attrs: {
      "prop": "limitMemberTags",
      "label": "限定会员标签"
    }
  }, [_c('i-select', {
    staticStyle: {
    ...
    attrs: {
      "multiple": true,
      "placeholder": "请选择需限定的会员标签"
    },
    model: {
      value: (_vm.includeMemberTagIdList),
      callback: function($$v) {
        _vm.includeMemberTagIdList = $$v
      },
      expression: "includeMemberTagIdList"
    }
  }, _vm._l((_vm.shopTagList), function(c) {
    return _c('i-option', {
      key: c.tagId,
      attrs: {
        "value": c.tagId
    ...
    staticClass: "l5"
  }, [_vm._v("\n\t\t\t\t\t\t\t设置后，表示仅有此会员标签（任意一个）的会员才能预约\n\t\t\t\t\t\t")])], 1) : _vm._e(), _vm._v(" "), (!_vm.member.memberId && _vm.displayView === 2) ? _c('form-item', {
    attrs: {
      "prop": "limitMemberTags",
      "label": "排除会员标签"
    }
    ...
    attrs: {
      "multiple": true,
      "placeholder": "请选择需排除的会员标签"
    },
    model: {
      value: (_vm.excludeMemberTagIdList),
      callback: function($$v) {
        _vm.excludeMemberTagIdList = $$v
      },
      expression: "excludeMemberTagIdList"
    }
  }, _vm._l((_vm.shopTagList), function(c) {
    return _c('i-option', {
      key: c.tagId,
    ...
    staticClass: "l5"
  }, [_vm._v("\n\t\t\t\t\t\t\t设置后，表示有此会员标签（任意一个）的会员不能预约\n\t\t\t\t\t\t")])], 1) : _vm._e(), _vm._v(" "), (_vm.displayView === 2) ? _c('form-item', {
    attrs: {
      "prop": "remark",
      "label": "备注"
    }
  ...
  })], 1) : _vm._e()], 2), _vm._v(" "), (_vm.displayView === 2) ? [_vm._m(0), _vm._v(" "), _c('i-form', {
    attrs: {
      "label-width": 130
    }
  }, [_c('form-item', {
  ...
  })], 2)] : _vm._e(), _vm._v(" "), (_vm.courseEditable) ? _c('div', {
    staticClass: "form-submit",
    staticStyle: {
      "padding-left": "130px"
    }
  ...
  }, [_vm._v("提交")]), _vm._v(" "), (_vm.ps && _vm.ps.psId && _vm.ps.enableWeektimes && _vm.ps.memberId && _vm.ps.beginDate && _vm.ps.endDate) ? _c('span', {
    staticClass: "l10"
  }, [_c('i-button', {
    on: {
      "click": function($event) {
  ...
  }, [_vm._v("批量预约私教课")])], 1) : _vm._e()], 1) : _vm._e()], 2) : _vm._e(), _vm._v(" "), (_vm.tab === 'trainPlan') ? [_c('i-button', {
    attrs: {
      "icon": "md-add",
      "type": "primary"
    },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [(_vm.logList && _vm.logList.length < 2) ? _c('div', {
    staticClass: "no-data"
  }, [_vm._v("\n\t\t暂无修改日志\n\t")]) : _vm._e(), _vm._v(" "), (_vm.logList && _vm.logList.length) ? _vm._l((_vm.logList), function(l, idx) {
    return (idx + 1 < _vm.logList.length) ? _c('section', [_c('div', {
      staticClass: "tit"
    }, [_vm._v("修改时间：" + _vm._s(l.createTime) + " "), _c('span', {
      staticClass: "l15"
    }, [_vm._v("修改人：" + _vm._s(l.createUname))])]), _vm._v(" "), _c('div', {
      staticClass: "mytbl"
    }, [_c('table', [_vm._m(0, true), _vm._v(" "), (l.courseId !== _vm.logList[idx + 1].courseId) ? _c('tr', [_c('td', [_vm._v("课程")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].courseName))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.courseName))])]) : _vm._e(), _vm._v(" "), (l.csName !== _vm.logList[idx + 1].csName) ? _c('tr', [_c('td', [_vm._v("排课名称")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].csName))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.csName))])]) : _vm._e(), _vm._v(" "), (l.lessonLevel !== _vm.logList[idx + 1].lessonLevel) ? _c('tr', [_c('td', [_vm._v("课程难度")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].lessonLevel))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.lessonLevel))])]) : _vm._e(), _vm._v(" "), (l.coachId !== _vm.logList[idx + 1].coachId) ? _c('tr', [_c('td', [_vm._v("教练")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].coachName))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.coachName))])]) : _vm._e(), _vm._v(" "), (l.coachLessonFeeType !== _vm.logList[idx + 1].coachLessonFeeType) ? _c('tr', [_c('td', [_vm._v("课时费类型")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Dict.GlCoachLessonFeeTypes[_vm.logList[idx + 1].coachLessonFeeType]))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Dict.GlCoachLessonFeeTypes[l.coachLessonFeeType]))])]) : _vm._e(), _vm._v(" "), (l.viceCoachIdList !== _vm.logList[idx + 1].viceCoachIdList) ? _c('tr', [_c('td', [_vm._v("助教")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].viceCoachIdList))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.viceCoachIdList))])]) : _vm._e(), _vm._v(" "), (l.groundId !== _vm.logList[idx + 1].groundId) ? _c('tr', [_c('td', [_vm._v("上课场地")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].groundName))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.groundName))])]) : _vm._e(), _vm._v(" "), (l.isNeedBooking !== _vm.logList[idx + 1].isNeedBooking) ? _c('tr', [_c('td', [_vm._v("是否需要预约")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].isNeedBooking ? '是' : '否'))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.isNeedBooking ? '是' : '否'))])]) : _vm._e(), _vm._v(" "), (l.maxStudents !== _vm.logList[idx + 1].maxStudents) ? _c('tr', [_c('td', [_vm._v("最多预约人数")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].maxStudents))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.maxStudents))])]) : _vm._e(), _vm._v(" "), (l.minStudents !== _vm.logList[idx + 1].minStudents) ? _c('tr', [_c('td', [_vm._v("最少预约人数")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].minStudents))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.minStudents))])]) : _vm._e(), _vm._v(" "), (l.allowMemberCheckin !== _vm.logList[idx + 1].allowMemberCheckin) ? _c('tr', [_c('td', [_vm._v("允许会员签到")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Dict.BoolValue[_vm.logList[idx + 1].allowMemberCheckin]))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Dict.BoolValue[l.allowMemberCheckin]))])]) : _vm._e(), _vm._v(" "), (l.maxMlBookings !== _vm.logList[idx + 1].maxMlBookings) ? _c('tr', [_c('td', [_vm._v("会员预约限制")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].maxMlBookings))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.maxMlBookings))])]) : _vm._e(), _vm._v(" "), (l.enableQueue !== _vm.logList[idx + 1].enableQueue) ? _c('tr', [_c('td', [_vm._v("是否启用排队")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Dict.BoolValue[_vm.logList[idx + 1].enableQueue]))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Dict.BoolValue[l.enableQueue]))])]) : _vm._e(), _vm._v(" "), (l.classMembers !== _vm.logList[idx + 1].classMembers && _vm.memberNameMap) ? _c('tr', [_c('td', [_vm._v("固定学员")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatMemberNames(_vm.logList[idx + 1].classMembers)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatMemberNames(l.classMembers)))])]) : _vm._e(), _vm._v(" "), (l.limitAges !== _vm.logList[idx + 1].limitAges || l.ageUnit !== _vm.logList[idx + 1].ageUnit) ? _c('tr', [_c('td', [_vm._v("学员年龄限制")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].limitAges) + " " + _vm._s(_vm.logList[idx + 1].ageUnit))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.limitAges) + " " + _vm._s(l.ageUnit))])]) : _vm._e(), _vm._v(" "), (l.custBookingPrice !== _vm.logList[idx + 1].custBookingPrice) ? _c('tr', [_c('td', [_vm._v("客户付费预约价格")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].custBookingPrice))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.custBookingPrice))])]) : _vm._e(), _vm._v(" "), (l.cardPaymentConf !== _vm.logList[idx + 1].cardPaymentConf) ? _c('tr', [_c('td', [_vm._v("上课计费卡")]), _vm._v(" "), _c('td', [_c('pre', [_vm._v(_vm._s(_vm.formatCardPaymentConf(_vm.logList[idx + 1].cardPaymentConf)))])]), _vm._v(" "), _c('td', [_c('pre', [_vm._v(_vm._s(_vm.formatCardPaymentConf(l.cardPaymentConf)))])])]) : _vm._e(), _vm._v(" "), (l.beginDate !== _vm.logList[idx + 1].beginDate || l.endDate !== _vm.logList[idx + 1].endDate) ? _c('tr', [_c('td', [_vm._v("课程日期")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Util.formatDate(_vm.logList[idx + 1].beginDate)) + " ~ " + _vm._s(_vm.g.Util.formatDate(_vm.logList[idx + 1].endate)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Util.formatDate(l.beginDate)) + " ~ " + _vm._s(_vm.g.Util.formatDate(l.endDate)))])]) : _vm._e(), _vm._v(" "), (l.week1Times !== _vm.logList[idx + 1].week1Times) ? _c('tr', [_c('td', [_vm._v("课程时间【周一】")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(_vm.logList[idx + 1].week1Times)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(l.week1Times)))])]) : _vm._e(), _vm._v(" "), (l.week2Times !== _vm.logList[idx + 1].week2Times) ? _c('tr', [_c('td', [_vm._v("课程时间【周二】")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(_vm.logList[idx + 1].week2Times)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(l.week2Times)))])]) : _vm._e(), _vm._v(" "), (l.week3Times !== _vm.logList[idx + 1].week3Times) ? _c('tr', [_c('td', [_vm._v("课程时间【周三】")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(_vm.logList[idx + 1].week3Times)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(l.week3Times)))])]) : _vm._e(), _vm._v(" "), (l.week4Times !== _vm.logList[idx + 1].week4Times) ? _c('tr', [_c('td', [_vm._v("课程时间【周四】")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(_vm.logList[idx + 1].week4Times)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(l.week4Times)))])]) : _vm._e(), _vm._v(" "), (l.week5Times !== _vm.logList[idx + 1].week5Times) ? _c('tr', [_c('td', [_vm._v("课程时间【周五】")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(_vm.logList[idx + 1].week5Times)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(l.week5Times)))])]) : _vm._e(), _vm._v(" "), (l.week6Times !== _vm.logList[idx + 1].week6Times) ? _c('tr', [_c('td', [_vm._v("课程时间【周六】")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(_vm.logList[idx + 1].week6Times)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(l.week6Times)))])]) : _vm._e(), _vm._v(" "), (l.week7Times !== _vm.logList[idx + 1].week7Times) ? _c('tr', [_c('td', [_vm._v("课程时间【周日】")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(_vm.logList[idx + 1].week7Times)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.formatWeekTimes(l.week7Times)))])]) : _vm._e(), _vm._v(" "), (l.publicTime !== _vm.logList[idx + 1].publicTime) ? _c('tr', [_c('td', [_vm._v("发布时间")]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.logList[idx + 1].publicTime))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.publicTime))])]) : _vm._e()])])]) : _vm._e()
  }) : _vm._e(), _vm._v(" "), _c('div', {
    staticClass: "gray p15"
  }, [_vm._v("备注：团课排课修改日志从【2023-12-11】开始记录")])], 2)
},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('tr', [_c('th', {
    attrs: {
      "width": "120"
    }
  }, [_vm._v("修改对象")]), _vm._v(" "), _c('th', [_vm._v("修改前")]), _vm._v(" "), _c('th', [_vm._v("修改后")])])
}]}
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('i-option', {
      key: c.courseId,
      attrs: {
        "value": parseInt(c.courseId),
        "label": c.courseName + (c.status === 99 ? '【已停用】' : '')
      }
    }, [_vm._v(_vm._s(c.courseName)), (c.status === 99) ? _c('span', {
      staticClass: "red l5"
    }, [_vm._v("【已停用】")]) : _vm._e()])
  }), 1)], 1), _vm._v(" "), _c('form-item', {
    attrs: {
    ...
    model: {
      value: (_vm.cs.gcType),
      callback: function($$v) {
        _vm.$set(_vm.cs, "gcType", $$v)
      },
      expression: "cs.gcType"
    }
  }, _vm._l((_vm.g.Dict.GcTypes), function(g, idx) {
    return _c('i-option', {
      key: idx,
    ...
    staticClass: "tips"
  }, [_vm._v("（可选填，仅用于课表过滤显示）")])], 1), _vm._v(" "), _c('form-item', {
    attrs: {
      "prop": "csName",
      "label": "排课名称"
    }
  ...
  }, [_vm._v("\n\t\t\t\t\t\t\t\t\t如果设置了最少预约人数，并且有人预约了，当上课前*小时（预约设置中可设置）未达到最少预约人数，\n\t\t\t\t\t\t\t\t\t"), _c('ol', [_c('li', [_vm._v("系统将自动发送提醒给已约课的会员和上课教练")]), _vm._v(" "), _c('li', [_vm._v("到达上课开始时间时，如果本节课依然未到达最少预约人数，且尚未有签到，系统将自动取消本节课，并发送通知给会员和上课教练")])]), _vm._v(" "), _c('p'), _vm._v("\n\t\t\t\t\t\t\t\t\t举例：如果某节课上课时间是12:00 ~ 13:00，设置最少预约人数为2人，提前1小时发通知，\n\t\t\t\t\t\t\t\t\t"), _c('ol', [_c('li', [_vm._v("如果11:00:00时仅有1人预约，那么系统将发送提醒给预约会员（提示本节课将被取消）")]), _vm._v(" "), _c('li', [_vm._v("如果12:00:00时本节课预约人数还是1人，且尚未有签到，系统将在12:00:01开始取消本节课，并发送通知给会员和上课教练")])]), _vm._v(" "), _c('p'), _vm._v("\n\t\t\t\t\t\t\t\t\t注：会员或教练想要收到系统发出的微信消息提醒，必须关注系统公众号（健总管）或场馆授权公众号，并且至少登录过一次健总管微信版（自动完成微信绑定）。\n\t\t\t\t\t\t\t\t")])], 1) : _vm._e()], 1) : _vm._e(), _vm._v(" "), _c('form-item', {
    attrs: {
  ...
  }, [_vm._v("\n\t\t\t\t\t\t\t\t如果设置为是，会员可在手机端签到；\n\t\t\t\t\t\t\t\t"), _c('p'), _vm._v("否则，不能在手机端签到，只能由前台或教练代签到。\n\t            ")])], 1), _vm._v(" "), (_vm.cs.isNeedBooking) ? _c('form-item', {
    attrs: {
      "prop": "maxMlBookings",
      "label": "会员预约限制"
    }
  ...
  }, [_vm._v("\n\t\t\t\t\t\t\t\t设置后，表示仅有此会员标签（任意一个）才能预约\n\t\t\t\t\t\t\t")])], 1)], 1), _vm._v(" "), _c('div', {
    staticClass: "tit"
  }, [_vm._v("计费设置")]), _vm._v(" "), _c('i-form', {
    staticClass: "time-conf",
    attrs: {
      "label-width": 120
    }
    ...
    staticClass: "iconfont icon-alert icon orange"
  }), _vm._v(" 请设置会员参与本课需要使用的卡以及费用")]), _vm._v(" "), _c('div', {
    staticClass: "p5 mb15"
  }, [_vm._v("\n\t\t\t\t\t\t\t\t卡类别过滤： \n\t\t\t\t\t\t\t\t"), _c('i-select', {
    staticStyle: {
      "width": "250px"
    ...
    return ((_vm.cardPayments[c.cardId].checked || c.status === 0 || _vm.showPausedCards) && _vm.cardClassList.contains(c.cardClass)) ? _c('li', [_c('div', {
      staticClass: "name"
    }, [_c('checkbox', {
      on: {
        "on-change": function($event) {
  ...
  })], 2)]), _vm._v(" "), (_vm.g.data.shop.hasWxPay && _vm.cs.isNeedBooking) ? _c('form-item', {
    attrs: {
      "prop": "custBookingPrice",
      "label": "客户付费预约价格"
    }
  }, [_c('input-num', {
    staticClass: "-input-s",
    ...
    model: {
      value: (_vm.cs.custBookingPrice),
      callback: function($$v) {
        _vm.$set(_vm.cs, "custBookingPrice", $$v)
      },
      expression: "cs.custBookingPrice"
    }
  }), _vm._v("   元 / 节\n\t\t\t\t\t\t\t"), _c('help', {
    attrs: {
      "title": "帮助"
    }
  }, [_vm._v("\n\t\t\t\t\t\t\t\t开通在线支付后，且微官网启用了散客付费预约后此设置才生效，然后散客可在微官网直接在线付费预约团课（无需购买会员卡）\n\t\t\t\t\t\t\t")])], 1) : _vm._e()], 1), _vm._v(" "), _c('div', {
    staticClass: "tit"
  }, [_c('div', {
    staticClass: "l"
  }, [_vm._v("课程周期设置\n\t\t\t\t\t\t\t"), _c('help', [_vm._v("\n\t\t\t\t\t\t\t\t如要删除某时间段，可直接清空或点击编辑框右端的删除符号；时间段可复制粘贴。\n\t\t\t\t\t\t\t\t"), _c('p'), _vm._v("\n\t\t\t\t\t\t\t\t修改如下任何数据，都将触发系统重新排课生成课表，系统将按照如下逻辑执行：\n\t\t\t\t\t\t\t\t"), _c('ol', [_c('li', [_vm._v("系统先删除此排课之前生成的课 "), _c('span', {
    staticClass: "orange"
  }, [_vm._v("（有预约或签到的课除外")]), _vm._v("）")]), _vm._v(" "), _c('li', [_vm._v("系统再按照最新的课程日期和上课时间生成新的课表，"), _c('span', {
    staticClass: "orangered"
  }, [_vm._v("但是只从当天日期开始生成，之前的课不会自动生成")])])])])], 1), _vm._v(" "), _c('div', {
    staticClass: "r"
    ...
    staticClass: "orange l15"
  }, [_vm._v("（ 如果不填写，则表示立即发布生成的课；否则，按照填写的时间发布到手机端 ）")])], 1), _vm._v(" "), (_vm.cs.csId) ? [_c('div', {
    staticClass: "tit"
  ...
  }, [_vm._v("添加一节课")]) : _vm._e(), _vm._v(" "), (_vm.csId) ? _c('i-button', {
    on: {
      "click": _vm.viewLessonsByCs
    }
  }, [_vm._v("查看已生成的课")]) : _vm._e(), _vm._v(" "), (_vm.csId) ? _c('i-button', {
    on: {
      "click": _vm.viewChangeLog
    }
  }, [_vm._v("查看修改日志")]) : _vm._e()], 1) : _vm._e()], 1)])]), _vm._v(" "), _c('modal', {
    attrs: {
      "title": "添加一节课",
      "width": "600",
      "ok-text": "关闭",
    ...
    model: {
      value: (_vm.lessonsDlg),
      callback: function($$v) {
        _vm.lessonsDlg = $$v
      },
      expression: "lessonsDlg"
    }
  }, [_c('div', {
    staticClass: "cont"
  }, [_c('date-picker', {
    ...
    model: {
      value: (_vm.lessonQuery.beginDate),
      callback: function($$v) {
        _vm.$set(_vm.lessonQuery, "beginDate", $$v)
      },
      expression: "lessonQuery.beginDate"
    }
  }), _vm._v("\n\t\t\t\t~\n\t\t\t\t"), _c('date-picker', {
    staticClass: "datepick",
    attrs: {
    ...
    model: {
      value: (_vm.lessonQuery.endDate),
      callback: function($$v) {
        _vm.$set(_vm.lessonQuery, "endDate", $$v)
      },
      expression: "lessonQuery.endDate"
    }
  }), _vm._v("\n\t\t\t\t \n\t\t\t\t"), _c('i-button', {
    attrs: {
      "type": "primary"
    ...
    on: {
      "click": function($event) {
        return _vm.queryLessonsByCs(null)
      }
    }
  }, [_vm._v("确定")])], 1), _vm._v(" "), _c('div', {
    staticClass: "mytbl mt15"
  }, [_c('table', [_c('tr', [_c('th', [_vm._v("上课日期 ↑")]), _vm._v(" "), _c('th', [_vm._v("上课时间")]), _vm._v(" "), _c('th', [_vm._v("状态")]), _vm._v(" "), _c('th', [_vm._v("教练")]), _vm._v(" "), _c('th', [_vm._v("最多预约人数")]), _vm._v(" "), _c('th', [_vm._v("最少预约人数")]), _vm._v(" "), _c('th', [_vm._v("已预约人数")]), _vm._v(" "), _c('th', [_vm._v("已签到人数")]), _vm._v(" "), _c('th', [_vm._v("签到人")]), _vm._v(" "), _c('th', [_vm._v("签到时间")])]), _vm._v(" "), _vm._l((_vm.lessonList), function(l) {
    return _c('tr', [_c('td', [_vm._v(_vm._s(_vm.g.Util.formatDate(l.lessonDate)))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(_vm.g.Util.formatTime(l.beginTime)) + " ~ " + _vm._s(_vm.g.Util.formatTime(l.endTime)))]), _vm._v(" "), _c('td', {
      class: 'l-status-' + l.status
    }, [_vm._v(_vm._s(_vm.g.Dict.LessonStatus[l.status]))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.coachName))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.maxStudents))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.minStudents))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.bookings))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.checkins))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.checkinStaffName))]), _vm._v(" "), _c('td', [_vm._v(_vm._s(l.checkinTime))])])
  })], 2)]), _vm._v(" "), (_vm.lessonCount && _vm.lessonCount >= 1) ? _c('div', {
    staticClass: "page-nav"
  }, [_c('page', {
    attrs: {
      "total": _vm.lessonCount,
      "current": _vm.lessonQuery.page ? _vm.lessonQuery.page : 1,
      "page-size": 30,
      "show-total": "",
      "show-elevator": ""
    },
    on: {
      "on-change": _vm.queryLessonsByCs
    }
  })], 1) : _vm._e(), _vm._v(" "), _c('div', {
    attrs: {
      "slot": "footer"
    ...
    model: {
      value: (_vm.csChangeLogDlg),
      callback: function($$v) {
        _vm.csChangeLogDlg = $$v
      },
      expression: "csChangeLogDlg"
    }
  }, [(_vm.csChangeLogDlg && _vm.csId) ? [_c('course-schedule-change-log', {
    attrs: {
      "cs-id": parseInt(_vm.csId)
    }
  })] : _vm._e(), _vm._v(" "), _c('div', {
    staticClass: "center",
    attrs: {
